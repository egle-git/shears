...
public:
  double integrate(TH1* h, int istart, const char* opt){
    double r = 0;
    if(istart==0) {r+= h->GetBinContent(0); ++istart; }
  ...
  void run(){
    ...
    TFile* recoFile = getFile(histoDir, lepSel, getEnergy(), "Data", 
			      jetPtMin, jetEtaMax, "", "0");
    ...
    if(recoFile){
	  ...
	  if(!hReco){
	    std::cerr << "Histogram " << hnames[iInc]
    ...
    TFile* unfFile = TFile::Open(getUnfoldedFileName(unfoldDir, lepSel, hnames[1], algo,
						     jetPtMin, jetEtaMax, "_MGPYTHIA6_", false) + ".root");
    if(!unfFile || unfFile->IsZombie()){
      std::cerr << "Warning. File of the unfolded distribution of " << hnames[1] << "was not found.\n";
    } else{
      TH1* hUnfDataCentral;
      unfFile->GetObject("UnfDataCentral", hUnfDataCentral);
      if(!hUnfDataCentral){
	std::cerr << "Histogram " << "UnfDataCentral"
		  << " was not found in file "
		  << unfFile->GetName() << "\n";
      } else{
	...
	continue;
      ...
      obsLabels[obs] = obs;
      TString section = TString::Format("%s_%s", lepSel.c_str(), VAROFINTERESTZJETS[i].name.Data());
      ...
      int istart = (VAROFINTERESTZJETS[i].name.Index("JetPt_Zinc") > 0) ? nFirstBinsToSkip + 1 : 0;
	...
	if(!hReco){
      ...
      TFile* unfFile = TFile::Open(getUnfoldedFileName(unfoldDir, lepSel, VAROFINTERESTZJETS[i].name, algo,
						       jetPtMin, jetEtaMax, "_MGPYTHIA6_", false) + ".root");
...
//		    << "dSigma = " << dSigma << "\n";
	...
	for(unsigned nJets = 0; nJets < mults[iInc].size(); ++nJets){
	  if(mults[iInc][nJets]){
	    colHeaders.push_back(TString::Format("%s %d jet%s", multLabel[iInc],
						  nJets, ( nJets > 0 ? "" : "s")).Data());
	  }
	}//next nJets
	...
	const char* captions[2][2] = {
	  {"Check of the integrals of the reconstruction-level histograms of the measured observables inclusive in jet multiplicity.",
	   "Check of the integrals of the reconstruction-level  histograms of the measured observables excluive in jetx multiplicity."},
	  {"Check of the integrals of the differential cross sections inclusive in jet multiplicity.",
	   "Check of the integrals of the differential cross sections exclusive in jet multiplicity."}
	};
	const char* labels[2][2] = {
	  {"tab:tot_exc_xsec_reco", "tab:tot_inc_xsec_reco"},
	  {"tab:tot_exc_xsec_unf", "tab:tot_inc_xsec_unf"}
	};
	printTable(f, colHeaders, lineHeaders, vals, captions[ixsec][iInc], labels[ixsec][iInc]); 
      }
  }
}
  };
