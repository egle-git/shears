...
public:
  ...
  bool read(const char* fname, bool check = false){
    ...
    if(!f.good()){
	  std::cerr << "Error. Configuration file '"
		    << fname << "' was not found\n";
	  abort();
    }
    ...
    std::regex param_pat("[[:space:]]*([^=[:space:]]*)[[:space:]]*=[[:space:]]*([^=[:space:]]*)[[:space:]]*(#.*)?");
    std::string section;
    std::map<std::string, rec_t>::iterator rcd = configMap.insert(std::pair<std::string, rec_t>("*", rec_t())).first;
    int iline = 0;
	...
	if(debug) std::cout << "\tIt's a parameter line\n"
			    << "\tParameter name: " << sm[1]
			    << "\tParameter value: " << sm[2] << "\n";
  ...
  void dumpRetrieved(std::ostream& o) const{
	...
	for(auto p: r.second){
	  ...
	  auto ir = common.insert(std::pair<std::string, std::pair<std::string, bool> >(p.first, val_and_flag));
	  bool inserted = ir.second;
	  std::pair<std::string, bool>& common_rec = ir.first->second;
	  if(!inserted && common_rec.first != val_and_flag.first){//common contains a different value
	    common_rec.second = false;//the value is not common to all sections, invalidate it
	  }
	}
      }
   ...
   */
  template<typename T>
  bool get1(const char* section, const char* param, T& val) const{
    const bool debug = false;
    bool found = false;
    std::string sval = "";
    for(auto r: configMap){
	...
	if(it2 != it1->second.end()){
	  std::stringstream buf;
	  if(sval != it2->second){
	    std::cerr << "Bug found: code uses inconsistent default value for parameter "
  ...
  template<typename T>
  T get(const char* section, const char* param, const T& defaultVal) const{
    T val = defaultVal;
  ...
  template<typename T>
  T get(const char* section, const char* param) const{
    T val;
