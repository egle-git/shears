...
int main(int argc, char **argv)
{
    ...
    try {
        ...
        std::string output_dir = input_dir + "/signal-over-background/";
        ...
        if (opt.map.count("histogram-name") > 0) {
            ...
            signal_entry.add_histograms(histogram_names);
        ...
        for (const std::string &name : histogram_names) {
            ...
            std::unique_ptr<TH1> signal = signal_entry.get(name, lumi);

            if (signal != nullptr) {
                signal->SetMarkerStyle(20);
                signal->SetMarkerColor(kGreen + 3);
                signal->SetLineColor(kGreen + 3);

                signal->GetYaxis()->SetLabelSize(0.04);
                signal->GetYaxis()->SetLabelOffset(0.002);
                signal->GetYaxis()->SetTitle("# Events");
                signal->GetYaxis()->SetTitleSize(0.04);
                signal->GetYaxis()->SetTitleOffset(1.32);
            ...
            std::unique_ptr<TH1> ratio = signal_entry.get(name, lumi);
            std::unique_ptr<TH1> background = background_entry.get(name, lumi);

            if (ratio != nullptr && background != nullptr) {
                upper.SetBottomMargin(0.);

                ratio->Divide(background.get());
                ...
                ratio->GetYaxis()->SetTitle("Signal/Background");
            ...
            signal_entry.reset_drawing_state();
            background_entry.reset_drawing_state();
        }
