...
void createInclusivePlots(bool doNormalized, double lumi, TString outputFileDir,
			  TString outputFileName, TH1 *hUnfData,
			  vector<TH2*> hCov, TH2 *hCovSyst,
...
void Combination(TString unfoldDir, TString combDir, TString algo, 
    ...
    for (int i = start; i < end; ++i) {
        ...
        if (!fDE->IsOpen()) {
            std::cerr << "\nError: file " << unfoldDir + "DE" + commonName << " does not exist." << std::endl;
            std::cerr << "       You can create it using\n\t ./runUnfoldingZJets lepSel=DE variable=" << variable << std::endl;
        ...
        if (!fDMu->IsOpen()) {
            std::cerr << "\nError: file " << unfoldDir + "DMu" + commonName << " does not exist." << std::endl;
            std::cerr << "       You can create it using\n\t ./runUnfoldingZJets lepSel=DMu variable=" << variable << std::endl;
            std::cerr << "Skipping variable " << variable << ".\n" << std::endl;
            continue;
        }
        //---------------------------------------------------------------------
	...
	for(unsigned ilep = 0; ilep < 2; ++ilep){
	  ...
	  for(unsigned iunc = 0; iunc < kUncCnt; ++iunc){
	    ...
	    if(!covariances[ilep][iunc]){
	      std::cerr << "Error. TH2 " << covName(iunc)
			<< " was not found in file " << f[ilep]->GetName()
			<< ". This uncertainty will be missing.\n";
	    }
	  }
	}
	...
	crossSectionPlot = makeCrossSectionPlot("", lumi, variable, doNormalized,
						hCombination, covuxaxb[kTotSys],
						predictions);
	...
	saveCanvas(crossSectionPlot, combDir, outputFileName);
	  ...
	  if(verbosity > 0) std::cout << hCombination->GetBinContent(i) << std::endl;
            tempunc = sqrt(covuxaxb[0]->GetBinContent(i,i) + covuxaxb[kTotSys]->GetBinContent(i,i));
	...
	if(verbosity > 0){
		...
		for (unsigned j = 0; j < covuxaxb.size(); ++j) {
		    ...
		    std::cout << " +/- " << sqrt(covuxaxb[j]->GetBinContent(i,i))*100./hCombination->GetBinContent(i) << "%";
		}
		std::cout << std::endl;
	    }
	}
	
        createTable(outputFilePath + "_withLERS", "", variable, doNormalized,
		    hCombination, covuxaxb, true, true);

        createTable(outputFilePath,               "", variable, doNormalized,
		    hCombination, covuxaxb, false, true);
	
        if (variable.Index("ZNGoodJets_Zexc") >= 0) {
		    createInclusivePlots(doNormalized, lumi, combDir, outputFileName,
				 hCombination, covuxaxb, covuxaxb[kTotSys], predictions);
        }
	...
	for(unsigned ilep = 0; ilep < 2; ++ilep){
	  for(unsigned iunc = 0; iunc < kUncCnt; ++iunc){
	    if(covariances[ilep][iunc]) covariances[ilep][iunc]->Write(covName(iunc, lepLabels[ilep]));
	  }
	}
        //--- Close all files ---
...
void createInclusivePlots(bool doNormalized, double lumi, 
    ...
    if(hCov.size() != kUncCnt){
      std::cerr << "Bug found in " << __FILE__ << ":" << __LINE__
		<< ": hCov vector has an unexpected size: "
		<< hCov.size() << " instead of " << kUncCnt
		<< "! Aborts.\n";
      abort();
    }
    ...
    TCanvas *crossSectionPlot = makeCrossSectionPlot(TString(""), lumi, TString("ZNGoodJets_Zinc"),
						     doNormalized, hInc, hCovInc[kTotSys],
						     predictions);
    ...
    createTable(outputFileDir + "/" + outputFileName + "_withLERS",
		"", TString("ZNGoodJets_Zinc"), doNormalized, hInc, hCovInc, true, true);
    createTable(outputFileDir + "/" + outputFileName,
		"", TString("ZNGoodJets_Zinc"), doNormalized, hInc, hCovInc, false, true);
}
