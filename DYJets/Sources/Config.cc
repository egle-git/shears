...
Config::Config(const char *filename) { read(filename); }

void Config::print(std::ostream &o)
{
    o << "#### vjets config file automatically generated on the ";
    o << __DATE__ << " at " << __TIME__ << std::endl;
    for (std::map<std::string, std::string>::const_iterator it = table_.begin(); it != table_.end();
         ++it) {
...
bool Config::get(const char *key, bool &value) { return get<bool>(key, value); }

bool Config::get(const char *key, int &value) { return get<int>(key, value); }

bool Config::get(const char *key, float &value) { return get<float>(key, value); }

bool Config::get(const char *key, double &value) { return get<double>(key, value); }

bool Config::get(const char *key, std::string &value) { return get<std::string>(key, value); }
...
bool Config::getB(const char *key, bool defaultValue) { return get2<bool>(key, defaultValue); }

int Config::getI(const char *key, int defaultValue) { return get2<int>(key, defaultValue); }
...
float Config::getF(const char *key, float defaultValue) { return get2<float>(key, defaultValue); }
...
template <typename T> void Config::convert(const T &x, std::string &s) const
{
    std::stringstream ss;
    ss << x;
    s = ss.str();
...
template <typename T> void Config::convert(const std::vector<T> &x, std::string &s) const
{
    s = "";
...
void Config::convert(const std::string &str, float &x) const { x = strtod(str.c_str(), 0); }

void Config::convert(const std::string &str, double &x) const { x = strtod(str.c_str(), 0); }

void Config::convert(const std::string &str, bool &x) const
{
    x = strtol(str.c_str(),
               0,
               0); // handles 0, 1. It will give also the corret value (0) for f(alse) and n(o)
    if (str.size() > 0 && (str[0] == 't' || str[0] == 'T' || str[0] == 'y' || str[0] == 'Y'))
        x = true;
}

void Config::convert(const std::string &str, int &x) const { x = strtol(str.c_str(), 0, 0); }

void Config::convert(const std::string &str, Long64_t &x) const { x = strtoll(str.c_str(), 0, 0); }

void Config::convert(const std::string &str, std::string &x) const { x = str; }

template <typename T> void Config::convert(const std::string &str, std::vector<T> &x) const
{
    x.clear();
    int pos = 0;
    std::string s = str;
...
std::string Config::tokenize(const std::string &s, const std::string &delim, int &pos) const
{
    if (pos < 0) return "";
    int pos0 = pos;
    int len = s.size();
...
template <typename T> bool Config::get(const char *key, T &value)
{
    ...
    if (it != table_.end()) {
        ...
        std::cerr << "         will use " << value_
                  << " as default value or the command line option.\n";
        std::string key_(key);
        table_[key_] = value_;
        return false;
    }
...
template <typename T> T Config::get2(const char *key, const T &defaultValue)
{
...
template <typename T> void Config::set(const char *key, const T &value)
{
    std::string sVal;
    convert(value, sVal);
    T prevValue;
...
template void Config::set<bool>(const char *key, const bool &value);
template void Config::set<int>(const char *key, const int &value);
template void Config::set<Long64_t>(const char *key, const Long64_t &value);
template void Config::set<float>(const char *key, const float &value);
template void Config::set<double>(const char *key, const double &value);
template void Config::set<std::string>(const char *key, const std::string &value);
template void Config::set<std::vector<int>>(const char *key, const std::vector<int> &value);
template void Config::set<std::vector<float>>(const char *key, const std::vector<float> &value);
template void Config::set<std::vector<double>>(const char *key, const std::vector<double> &value);
template void Config::set<std::vector<std::string>>(const char *key,
