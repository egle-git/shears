...
JetCorrectorParameters const & JetCorrectorParametersCollection::operator[]( key_type k ) const {
  ...
  std::string tmpexcept = "InvalidInput cannot find key ";
  tmpexcept += static_cast<int>(k) + " in the JEC payload, this usually means you have to change the global tag\n";
  throwException(1,tmpexcept);
}
...
JetCorrectorParametersCollection::findKey( std::string const & label ) const {
  ...
  std::string tmpexcept = "InvalidInput Cannot find label "+ label+ "\n";
  throwException(1,tmpexcept);
