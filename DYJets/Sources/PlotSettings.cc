...
void setAndDrawTPad(TString canvasName, TPad *plot, int plotNumber, int numbOfGenerator)
{
    ...
    if (plotNumber == 1) {
        ...
        y0 = 0.99;
        //	 y1 = 0.99 - htop * (1 + margin0 + margin1);
        y1 = 0.99 - htop;
    ...
    if (plotNumber == 1 && (canvasName.Index("Eta") < 0 && canvasName.Index("AbsRapidity") < 0 &&
                            canvasName.Index("DPhi") < 0))
        plot->SetLogy();
...
void customizeCentral(TGraphAsymmErrors *grCentral, TLegend *legend, TString legText)
{
    ...
    grCentral->GetXaxis()->SetLabelFont(43);
...
TGraphAsymmErrors *createGrFromHist(const TH1 *h)
{
    ...
    TGraphAsymmErrors *gr = new TGraphAsymmErrors(
        nPoints, &xCoor[0], &yCoor[0], &xErr[0], &xErr[0], &yErr[0], &yErr[0]);
...
TGraphAsymmErrors *createRatioGraph(const TGraphAsymmErrors *grCentral)
{
    ...
    TGraphAsymmErrors *grCentralRatio =
        new TGraphAsymmErrors(nPoints, xCoor, yCoor, xErr, xErr, yErrL, yErrH);
...
TGraphAsymmErrors *createGenToCentral(const TH1 *gen, const TGraphAsymmErrors *grCentral)
{
    ...
    TGraphAsymmErrors *grGenToCentral = new TGraphAsymmErrors(
        nPoints, &xCoor[0], &yCoor[0], &xErr[0], &xErr[0], &yErr[0], &yErr[0]);
    return grGenToCentral;
}

TGraphAsymmErrors *createScaleSystGraph(TString sample,
                                        TString lepSel,
                                        TString variable,
                                        const TGraphAsymmErrors *grGenToCentral)
{
    ...
    TFile *fDE;
    if (lepSel == "DE" || lepSel == "") {
        fDE = new TFile(histoDir + "/DE_13TeV_" + sample +
    ...
    if (lepSel == "DMu" || lepSel == "") {
        fDMu = new TFile(histoDir + "/DMu_13TeV_" + sample +
                         "_TrigCorr_1_Syst_0_JetPtMin_30_JetEtaMax_24.root");
    ...
    for (int i(0); i < nPoints; i++) {
        ...
        if (lepSel == "" && grDE && grDMu) {
            ...
            grDE->GetPoint(i, xMeanDE[i], yMeanDE[i]);
            yErrUp[i] += pow(
                ((grDMu->GetErrorYhigh(i) + grDE->GetErrorYhigh(i)) / (yMeanDMu[i] + yMeanDE[i])) *
                    yCoor[i],
                2);
            yErrDown[i] += pow(
                ((grDMu->GetErrorYhigh(i) + grDE->GetErrorYhigh(i)) / (yMeanDMu[i] + yMeanDE[i])) *
    ...
    TGraphAsymmErrors *grScaleSyst =
        new TGraphAsymmErrors(nPoints, xCoor, yCoor, xErr, xErr, yErrDown, yErrUp);
    delete[] xCoor;
    delete[] yCoor;
    delete[] xErr;
    delete[] yErrDown;
...
TGraphAsymmErrors *createGenevaIncScaleSystGraph(TString sample,
                                                 TString lepSel,
                                                 TString variable,
                                                 const TGraphAsymmErrors *grGenToCentral)
{
...
TGraphAsymmErrors *
createNNLOScaleSystGraph(TString lepSel, TString variable, const TGraphAsymmErrors *grGenToCentral)
{
    int nPoints = grGenToCentral->GetN();
    double *xCoor = new double[nPoints];
    double *yCoor = new double[nPoints];
    ...
    if (lepSel == "DE" || lepSel == "DMu" || lepSel == "") {
        fnnlo[0] = new TFile(
            histoDir + "/DMu_13TeV_DYJets_ZjNNLO_TrigCorr_1_Syst_0_JetPtMin_30_JetEtaMax_24.root");
        if (!fnnlo[0] || fnnlo[0]->IsZombie()) {
            std::cerr << "Fatal error. Histo file for ZjNNLO prediction, " << histoDir + "NNLO.root"
                      << " was not found.\n";
        }
        fnnlo[1] = new TFile(
            histoDir +
            "/DMu_13TeV_DYJets_ZjNNLO_Scaleup_TrigCorr_1_Syst_0_JetPtMin_30_JetEtaMax_24.root");
        ...
        fnnlo[2] = new TFile(
            histoDir +
            "/DMu_13TeV_DYJets_ZjNNLO_Scaledn_TrigCorr_1_Syst_0_JetPtMin_30_JetEtaMax_24.root");
        if (!fnnlo[2] || fnnlo[2]->IsZombie()) {
            std::cerr
                << "Fatal error. Histo file for scale down uncertainties of ZjNNLO prediction, "
                << histoDir + "NNLO.root"
                << " was not found.\n";
        }
    }
    ...
    for (int i(0); i < nPoints; i++) {
        ...
        if (lepSel == "DMu" || lepSel == "DE" || lepSel == "") {
            yErrUp[i] += pow((hnnlo[1]->GetBinContent(i + 1) - hnnlo[0]->GetBinContent(i + 1)) /
                                 hnnlo[0]->GetBinContent(i + 1) * yCoor[i],
                             2);
            yErrDown[i] += pow((hnnlo[0]->GetBinContent(i + 1) - hnnlo[2]->GetBinContent(i + 1)) /
                                   hnnlo[0]->GetBinContent(i + 1) * yCoor[i],
                               2);
        }
...
TGraphAsymmErrors *createPDFSystGraph(TString sample,
                                      TString lepSel,
                                      TString variable,
                                      const TGraphAsymmErrors *grGenToCentral,
                                      const TGraphAsymmErrors *grGen3ScaleSyst)
{
    int nPoints = grGenToCentral->GetN();
    double *xCoor = new double[nPoints];
    double *yCoor = new double[nPoints];
...
// TGraphAsymmErrors* createPDFSystGraph(TString lepSel, TString variable, const TGraphAsymmErrors
// *grGenToCentral)
//{
//    int nPoints = grGenToCentral->GetN();
//    double *xCoor    = new double[nPoints];
...
//    if (lepSel == "DE" || lepSel == "") {
//      //        fDE = new
//      TFile("HistoFilesUnc/DE_13TeV_DYJets_UNFOLDING_TrigCorr_1_Syst_0_JetPtMin_30_JetEtaMax_24.root");
//        fDE = new TFile(histoDir +
//        "/DE_13TeV_DYJets_UNFOLDING_TrigCorr_1_Syst_0_JetPtMin_30_JetEtaMax_24.root");
//    }
//
//    TFile *fDMu;
//    if (lepSel == "DMu" || lepSel == "") {
//        fDMu = new TFile(histoDir +
//        "/DMu_13TeV_DYJets_UNFOLDING_TrigCorr_1_Syst_0_JetPtMin_30_JetEtaMax_24.root");
//    }
//
//    TGraphAsymmErrors *grDE, *grDMu;
//    if (lepSel == "DE" || lepSel == "") {
...
//            grDMu->GetPoint(i, xMeanDMu[i], yMeanDMu[i]);
//            grDE->GetPoint(i, xMeanDE[i], yMeanDE[i]);
//            yErrUp[i] += pow(((grDMu->GetErrorYhigh(i) + grDE->GetErrorYhigh(i)) / (yMeanDMu[i] +
//            yMeanDE[i])) * yCoor[i], 2);
//            yErrDown[i] += pow(((grDMu->GetErrorYhigh(i) + grDE->GetErrorYhigh(i)) / (yMeanDMu[i]
//            + yMeanDE[i])) * yCoor[i], 2);
//        }
//
//    }
//
//    TGraphAsymmErrors *grPDFSyst = new TGraphAsymmErrors(nPoints, xCoor, yCoor, xErr, xErr,
//    yErrDown, yErrUp);
...
void customizeRatioGraph(TH1 *hAxis,
                         TGraphAsymmErrors *gen,
                         TGraphAsymmErrors *gScale,
...
void configYaxis(TH1 *grCentralSyst, TH1 *gen1, TH1 *gen2, TH1 *gen3)
{
    ...
    //    if(grCentralSyst) grCentralSyst->GetYaxis()->SetRangeUser(0.2*minimumToPlot,
    //    5*maximumToPlot);
...
void makeCrossSectionPlot(const char *variable, const char *ref)
{
    ...
    TString path = getUnfoldedFileName(
    ...
    if (TString(ref).CompareTo("data", TString::kIgnoreCase) == 0) {
        ...
        if (!fdata) {
            std::cerr << "Fatal error. Failed to open unfolded data file " << (path + ".root")
                      << "\n";
            abort();
        }
        ...
        hRef = (TH1 *)getGenHistos(
            std::vector<std::string>(1, ref), lepSel.c_str(), variable, true, true)[0];
        if (hRef) {
    ...
    if (hRef) {
        ...
        TCanvas *crossSectionPlot =
            makeCrossSectionPlot(lepSel, lumi, variable, doNormalized, hRef, hCov, predictions);
        saveCanvas(crossSectionPlot, dir, fname);
    } else {
        std::cerr << "Warning: no reference histogram was found for variable " << variable
                  << ", channel " << lepSel << "\n";
    }
    delete hRef;
    delete hCov;
...
TCanvas *makeCrossSectionPlot(TString lepSel,
                              double lumi,
                              TString variable,
                              bool doNormalized,
                              TH1 *hStat,
                              ...
                              std::vector<std::string> gens)
{
    ...
    struct GenRcd
    {
        GenRcd(const std::string &n,
               TH1 *h,
               TGraphAsymmErrors *ratio = 0,
               TGraphAsymmErrors *pdf = 0,
               TGraphAsymmErrors *scale = 0)
            : name(n), histo(h), dataRatio(ratio), dataRatioPDFSyst(pdf), dataRatioScaleSyst(scale)
        {
        }
    ...
    for (auto g : hGens) {
        ...
        if (!alignRanges(hStat->GetXaxis(), g->GetXaxis())) {
            std::cerr << "Fatal error. X-axis range of " << hStat->GetName() << " and "
                      << g->GetName() << " are inconsistent. Aborts. (" << __FILE__ << ":"
                      << __LINE__ << ")\n";
        ...
        if (TString(display.c_str()).BeginsWith("on-top-of_")) {
            ...
            if (std::find(gens.begin(), gens.end(), altGen) == gens.end()) {
                std::cerr << "Warning: prediction " << altGen << " indicated in the parameter "
                          << display << " is not listed by the parameter predictions. "
    ...
    if (hCovSyst) {
        ...
        for (int i = 1; i <= nBins; ++i) {
            hSyst->SetBinError(i,
                               sqrt(pow(hStat->GetBinError(i), 2) + hCovSyst->GetBinContent(i, i)));
        }
    }
    ...
    for (auto &f : ratioFrames) {
        for (auto &g : f) {
            ...
            if (showSys == 1) {
                ...
                g.dataRatioPDFSyst =
                    createPDFSystGraph(g.name, lepSel, variable, g.dataRatio, g.dataRatioScaleSyst);
                ...
                g.dataRatioScaleSyst =
                    createGenevaIncScaleSystGraph(g.name, lepSel, variable, g.dataRatio);
            }
        }
    }
    ...
    TCanvas *plots =
        new TCanvas(canvasName,
                    hStat->GetTitle(),
                    600,
                    400 * (1 + padHeightRatio * ratioYaxisExtend * ratioFrames.size()));
#else
    ...
    for (auto &g : mainFrame) {
        ...
        if (!g.histo) continue;
        customizeGenHist(
            g.histo, igen + 1, legend, TString::Format("%s", g.histo->GetZaxis()->GetTitle()));
        g.histo->SetStats(0);
    ...
    if (TString(hStat->GetZaxis()->GetTitle()).BeginsWith("data", TString::kIgnoreCase) ||
        ...
        if (lumi > 0)
            lumiLabel.DrawLatex(
                0.13, 0.95 - 0.045, TString::Format("%.3g fb^{-1} (13 TeV)", lumi / 1000.));
    } else {
        cmsLabel.DrawLatex(0.13, 0.95, "MC study");
        if (isPrel) prelimLabel.DrawLatex(0.13, 0.95 - 0.045, "Preliminary");
    }

    // cmsLabel->DrawLatex(0.18,0.21-0.05,"anti-k_{T} (R = 0.4) Jets");
    ...
    //	xlabel = 0.44;
    //	ylabel = 0.75;
    //	//	descLabel.DrawLatex(0.44,0.7,"anti-k_{T} (R = 0.4) Jets");
    //} else
    if (canvasName.Contains("JZB") && !canvasName.Contains("JZB_ptLow") &&
        !canvasName.Contains("JZB_ptHigh")) {
        xlabel = 0.4;
        ylabel = 0.23;
        ...
        xlabel = 0.18;
        ylabel = 0.23;
        //	descLabel.DrawLatex(0.18,0.21-0.05,"anti-k_{T} (R = 0.4) Jets");
    }
    descLabel.DrawLatex(xlabel, ylabel - 0.05, "anti-k_{T} (R = 0.4) Jets");
    ...
    if (canvasName.Contains("FirstJetPt50")) {
        ...
        descLabel.DrawLatex(
            xlabel, ylabel - 0.11, "p_{T}^{Z} > 150 GeV, p_{T}^{jet} > 30 GeV, |y^{jet}| < 2.4 ");
    } else if (canvasName.Contains("ZPt300")) {
        descLabel.DrawLatex(
            xlabel, ylabel - 0.11, "p_{T}^{Z} > 300 GeV, p_{T}^{jet} > 30 GeV, |y^{jet}| < 2.4 ");
    } else if (canvasName.Contains("DifJetRapidityl2")) {
        descLabel.DrawLatex(xlabel,
                            ylabel - 0.11,
                            "p_{T}^{jet} > 30 GeV, |y^{jet}| < 2.4, |y_{jet1}-y_{jet2}| > 2 ");
        ...
        descLabel.DrawLatex(
            xlabel,
            ylabel - 0.11,
            "p_{T}^{Z} > 150 GeV, p_{T}^{jet} > 30 GeV, |y^{jet}| < 2.4, H_{T}^{jet} > 300 GeV ");
        //} else if (canvasName.Contains("Vis")){
        //	descLabel.DrawLatex(xlabel, 0.7-0.06,"p_{T}^{#mu} > 20 GeV, p_{T}^{jet} > 30 GeV,
        //|#eta^{jet}| < 2.4 ");
    } else {
    ...
    if (lepSel == "") {
        descLabel.DrawLatex(xlabel, ylabel - 0.17, "Z/#gamma*#rightarrow ll channel");
        if (canvasName.Contains("inc1"))
            descLabel.DrawLatex(
                xlabel, ylabel - 0.17, "Z/#gamma*#rightarrow ll channel, N_{jets} #geq 1");
        if (canvasName.Contains("inc2"))
            descLabel.DrawLatex(
                xlabel, ylabel - 0.17, "Z/#gamma*#rightarrow ll channel, N_{jets} #geq 2");
        if (canvasName.Contains("inc3"))
            descLabel.DrawLatex(
                xlabel, ylabel - 0.17, "Z/#gamma*#rightarrow ll channel, N_{jets} #geq 3");
        if (canvasName.Contains("JZB"))
            descLabel.DrawLatex(
                xlabel, ylabel - 0.17, "Z/#gamma*#rightarrow ll channel, N_{jets} #geq 1");
        if (canvasName.Contains("JZB_ptHigh"))
            descLabel.DrawLatex(
                xlabel,
                ylabel - 0.17,
                "Z/#gamma*#rightarrow ll channel, N_{jets} #geq 1, p_{T}(Z) > 50 GeV");
        if (canvasName.Contains("JZB_ptLow"))
            descLabel.DrawLatex(
                xlabel,
                ylabel - 0.17,
                "Z/#gamma*#rightarrow ll channel, N_{jets} #geq 1, p_{T}(Z) #leq 50 GeV");
    }

    else if (lepSel == "DMu") {
        descLabel.DrawLatex(xlabel, ylabel - 0.17, "Z/#gamma*#rightarrow #mu#mu channel");
        if (canvasName.Contains("inc1"))
            descLabel.DrawLatex(
                xlabel, ylabel - 0.17, "Z/#gamma*#rightarrow #mu#mu channel, N_{jets} #geq 1");
        if (canvasName.Contains("inc2"))
            descLabel.DrawLatex(
                xlabel, ylabel - 0.17, "Z/#gamma*#rightarrow #mu#mu channel, N_{jets} #geq 2");
        if (canvasName.Contains("inc3"))
            descLabel.DrawLatex(
                xlabel, ylabel - 0.17, "Z/#gamma*#rightarrow #mu#mu channel, N_{jets} #geq 3");
        if (canvasName.Contains("JZB"))
            descLabel.DrawLatex(
                xlabel, ylabel - 0.17, "Z/#gamma*#rightarrow #mu#mu channel, N_{jets} #geq 1");
        if (canvasName.Contains("JZB_ptHigh"))
            descLabel.DrawLatex(
                xlabel,
                ylabel - 0.17,
                "Z/#gamma*#rightarrow #mu#mu channel, N_{jets} #geq 1, p_{T}(Z) > 50 GeV");
        if (canvasName.Contains("JZB_ptLow"))
            descLabel.DrawLatex(
                xlabel,
                ylabel - 0.17,
                "Z/#gamma*#rightarrow #mu#mu channel, N_{jets} #geq 1, p_{T}(Z) #leq 50 GeV");
    }

    else if (lepSel == "DE") {
        descLabel.DrawLatex(xlabel, ylabel - 0.17, "Z/#gamma*#rightarrow ee channel");
        if (canvasName.Contains("inc1"))
            descLabel.DrawLatex(
                xlabel, ylabel - 0.17, "Z/#gamma*#rightarrow ee channel, N_{jets} #geq 1");
        if (canvasName.Contains("inc2"))
            descLabel.DrawLatex(
                xlabel, ylabel - 0.17, "Z/#gamma*#rightarrow ee channel, N_{jets} #geq 2");
        if (canvasName.Contains("inc3"))
            descLabel.DrawLatex(
                xlabel, ylabel - 0.17, "Z/#gamma*#rightarrow ee channel, N_{jets} #geq 3");
        if (canvasName.Contains("JZB"))
            descLabel.DrawLatex(
                xlabel, ylabel - 0.17, "Z/#gamma*#rightarrow ee channel, N_{jets} #geq 1");
        if (canvasName.Contains("JZB_ptHigh"))
            descLabel.DrawLatex(
                xlabel,
                ylabel - 0.17,
                "Z/#gamma*#rightarrow ee channel, N_{jets} #geq 1, p_{T}(Z) > 50 GeV");
        if (canvasName.Contains("JZB_ptLow"))
            descLabel.DrawLatex(
                xlabel,
                ylabel - 0.17,
    ...
    //    std::string strYtitle = hGen1 ?  getYaxisTitle(doNormalized, hGen1) :
    //    getYaxisTitle(doNormalized, hStat);
    ...
    for (auto &f : ratioFrames) {
        ...
        for (auto &g : f) {
            ++igen;
            customizeRatioGraph(mainGen ? hAxis : 0,
                                g.dataRatio,
                                g.dataRatioScaleSyst,
                                g.dataRatioPDFSyst,
                                ipad - 1,
