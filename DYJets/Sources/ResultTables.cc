...
void createTable(TString outputFilePath, TString lepSel, TString variable, bool doNormalized,
		 TH1 *hCombination, vector<TH2*> &covuxaxb, bool withLERS, bool forceNoLumiSep){
  ...
  //--- print out break down of errors ---
    ...
    for (int i = start; i <= nBins; ++i) {
        ...
        //numbers.Form("%#.3g", xs);
	//table += numbers + " & ";
	//	double totUnc = sqrt(covuxaxb[0]->GetBinContent(i,i) + covxaxbSyst->GetBinContent(i,i));
	double totUnc = sqrt(covuxaxb[0]->GetBinContent(i,i) + covuxaxb[kTotSys]->GetBinContent(i,i));
	if(sepLumiUnc){
	...
	// Unf uncertainty
	if(covuxaxb[kUnfSys]) numbers.Form("%#.2g", sqrt(covuxaxb[kUnfSys]->GetBinContent(i,i))*100./xs);
	else numbers = "-";
