...
int RocRes::getEtaBin(double feta) const { return getBin(feta, NETA, BETA); }

int RocRes::getNBinDT(double v, int H) const { return getBin(v, NTRK, dtrk[H]); }

int RocRes::getNBinMC(double v, int H) const { return getBin(v, NTRK, ntrk[H]); }
...
void RocRes::init(std::string filename)
{
    ...
    while (std::getline(in, s)) {
        ...
        if (s.substr(0, 4) == "RMIN")
            ss >> tag >> NMIN;
        else if (s.substr(0, 4) == "RTRK")
            ss >> tag >> NTRK;
            ...
            if (var == 0)
                for (int i = 0; i < NTRK; ++i) ss >> rmsA[bin][i];
            if (var == 1)
                for (int i = 0; i < NTRK; ++i) ss >> rmsB[bin][i];
            if (var == 2)
            ...
            if (var == 3)
                for (int i = 0; i < NTRK; ++i) ss >> width[bin][i];
            if (var == 4)
                for (int i = 0; i < NTRK; ++i) ss >> alpha[bin][i];
            if (var == 5)
                for (int i = 0; i < NTRK; ++i) ss >> power[bin][i];
        } else if (s.substr(0, 1) == "T") {
            ss >> tag >> type >> sys >> mem >> isdt >> var >> bin;
            if (isdt == 0)
                for (int i = 0; i < NTRK + 1; ++i) ss >> ntrk[bin][i];
            if (isdt == 1)
                for (int i = 0; i < NTRK + 1; ++i) ss >> dtrk[bin][i];
        } else if (s.substr(0, 1) == "F") {
            ss >> tag >> type >> sys >> mem >> isdt >> var >> bin;
            if (var == 0) {
                if (isdt == 0)
                    for (int i = 0; i < NETA; ++i) ss >> kRes[i];
                if (isdt == 1)
                    for (int i = 0; i < NETA; ++i) ss >> kDat[i];
            }
        }
    }
...
double
RocOne::kScaleAndSmearMC(int Q, double pt, double eta, double phi, int n, double u, double w) const
{
...
double
RocOne::kScaleFromGenMC(int Q, double pt, double eta, double phi, int n, double gt, double w) const
{
    double k = kScaleMC(Q, pt, eta, phi);
...
RoccoR::RoccoR(std::string dirname) { init(dirname); }

void RoccoR::init(std::string dirname)
{
...
double
RoccoR::kGenSmear(double pt, double eta, double v, double u, RocRes::TYPE TT, int s, int m) const
{
...
double RoccoR::kScaleAndSmearMC(
    int Q, double pt, double eta, double phi, int n, double u, double w, int s, int m) const
{
    return RC[s][m].kScaleAndSmearMC(Q, pt, eta, phi, n, u, w);
}

double RoccoR::kScaleFromGenMC(
    int Q, double pt, double eta, double phi, int n, double gt, double w, int s, int m) const
{
