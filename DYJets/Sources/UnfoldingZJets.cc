...
void createInclusivePlots(bool doNormalized,
                          ...
                          TH1 *hUnfData,
                          TH2 *hCov[],
                          const std::vector<string> &predictions,
                          double integratedLumi);

// void createInclusivePlots(bool doNormalized, TString outputFileName, TString lepSel, TH1D
// *hUnfData, TH2D *hCov[], TH1D *hMadGenCrossSection, TH1D *hSheGenCrossSection);
// void createInclusivePlots(bool doNormalized, TString outputFileName, TString lepSel, TH1D
// *hUnfData, TH2D *hCov[], TH1D *hMadGenCrossSection);
...
//        int jetPtMin, int jetEtaMax, TString gen1, TString gen2, TString variable, bool
//        doNormalized)
void UnfoldingZJets(const SectionedConfig &unfCfg,
                    TString lepSel,
                    TString algo,
                    TString histoDir,
                    TString unfoldDir,
                    ...
                    int whichSyst)
{
    ...
    // 0 - central, 1 - PU up, 2 - PU down, 3 - JER up, 4 - JER down, 5 - LES up, 6 - LES down, 7 -
    // LER up, 8 - LER down
    ...
    for (int i = start; i < end; ++i) {
        ...
        if (Lumi)
            integratedLumi = Lumi->GetBinContent(1);
        else {
            cerr << "Error: Lumi histogram was not found.\n";
            return;
        }

        std::unique_ptr<TFile> fAltUnf(
            new TFile(histoDir + lepSel + "_13TeV_" + "DYJets_UNFOLDING_UNC" +
                      "_TrigCorr_1_Syst_0_JetPtMin_30_JetEtaMax_24.root"));

        variable = VAROFINTERESTZJETS[i].name;
        TString outputFileName = getUnfoldedFileName(
            unfoldDir, lepSel, variable, algo, jetPtMin, jetEtaMax, "_MGPYTHIA6_", doNormalized);
        ...
        if (fData[0]) {
            ...
            if (!hRecDataOdd) {
                std::cout << "Missing Odd histogram for " << variable << "\n";
            }
            if (!hRecDataEven) std::cout << "Missing Even histogram for " << variable << "\n";
        }

        //--- Get all histograms ---
        getAllHistos(variable,
                     hRecData,
                     fData,
                     hRecDYJets,
                     hGenDYJets,
        ...
        if (fAltUnf && withUnfUnc) {
            ...
            if (respDYJets[17] == 0) {
                std::cerr << "Response matrix was not found in the file " << fAltUnf->GetName()
                          << ". Aborts.\n";
                abort();
            }
        }
        ...
        for (unsigned short iSyst = 0; iSyst < nSysts; ++iSyst) {
            ...
            if (iSyst == 0 || iSyst == 1 || iSyst == 2 || iSyst == 5 || iSyst == 6 || iSyst == 11 ||
                iSyst == 12 || iSyst == 17)
                iBg = 0; // Central, JES, JER, LER, Sherpa
            else if (iSyst == 3 || iSyst == 4)
                iBg = iSyst - 2; // PU
            else if (iSyst == 7 || iSyst == 8 || iSyst == 9 || iSyst == 10)
                iBg = iSyst - 4; // XSec, LES
            else if (iSyst == 13 || iSyst == 14 || iSyst == 15 || iSyst == 16)
                iBg = iSyst - 6; // Lumi, SF

            TH1D *hRecDataMinusFakes = (TH1D *)hRecData[iData]->Clone(
                TString::Format("hBkgSubData_%s", hRecData[iData]->GetName()));
            ...
            std::cout << "Starting unfolding of " << variable << " " << name[iSyst] << " for  "
                      << lepSel << " channel."
                      << "\n";
        ...
        if (doNormalized) {
            for (int i = 0; i < nSysts; i++) {
                ...
                double totUnfData =
                    hUnfData[i]->Integral("width"); // normalize to central or itself?
        ...
        if (hUnfData[kJESup])
            hCov[kJES] = makeCovFromUpAndDown(
                hUnfData[0], hUnfData[kJESup], hUnfData[kJESdwn], covName(kJES), noNull);
        if (hUnfData[kPUup])
            hCov[kPU] = makeCovFromUpAndDown(
                hUnfData[0], hUnfData[kPUup], hUnfData[kPUdwn], covName(kPU), noNull);
        if (hUnfData[kJERup])
            hCov[kJER] = makeCovFromUpAndDown(
                hUnfData[0], hUnfData[kJERup], hUnfData[kJERdwn], covName(kJER), noNull);
        if (hUnfData[kXsecUp])
            hCov[kXsec] = makeCovFromUpAndDown(
                hUnfData[0], hUnfData[kXsecUp], hUnfData[kXsecDwn], covName(kXsec), noNull);
        if (hUnfData[kLESup])
            hCov[kLES] = makeCovFromUpAndDown(
                hUnfData[0], hUnfData[kLESup], hUnfData[kLESdwn], covName(kLES), noNull);
        if (hUnfData[kLERup])
            hCov[kLER] = makeCovFromUpAndDown(
                hUnfData[0], hUnfData[kLERup], hUnfData[kLERdwn], covName(kLER), noNull);
        if (hUnfData[kLumiUp])
            hCov[kLumi] = makeCovFromUpAndDown(
                hUnfData[0], hUnfData[kLumiUp], hUnfData[kLumiDwn], covName(kLumi), noNull);
        ...
        if (hUnfData[kSFup])
            hCov[kSF] = makeCovFromUpAndDown(
                hUnfData[0], hUnfData[kSFup], hUnfData[kSFdwn], covName(kSF), noNull);
        if (hUnfData[kAltUnf])
            hCov[kUnfSys] =
                makeCovFromUpAndDown(hUnfData[0], hUnfData[kAltUnf], 0, covName(kUnfSys), noNull);
        ...
        if (whichSyst < 0) {
            ...
            createTable(outputFileName + "_withLERS",
                        ...
                        hCov,
                        true);
            createTable(outputFileName, lepSel, variable, doNormalized, hUnfData[0], hCov, false);
        }
        ...
        // outputFileName.ReplaceAll("ZNGoodJets_Zexc", "ZNGoodJets_Zinc") + ".png &");

        gettimeofday(&t1, 0);
        std::cout << __FILE__ << ":" << __LINE__ << ": unfolding of  variable " << variable
                  << " for channel " << lepSel << " and systematic #" << whichSyst << " took "
                  << ((t1.tv_sec - t0.tv_sec) * 1e3 + (t1.tv_usec - t0.tv_usec) * 1e-3) << " ms.\n";
    ...
    f << "#Files automatically genertaed by UnfoldingZJets. The file will be overwritten at next "
         "execution.\n\n";
    unfCfg.dumpRetrieved(f);
}

void createSystPlots(TString outputFileName,
                     TString sysPlotDir,
                     TString variable,
                     TString lepSel,
                     TH1D *hUnfData[],
                     bool logy)
{
    ...
    for (int i = 1; i < 16; i += 2) {
        ...
        std::unique_ptr<TCanvas> c(
            new TCanvas(variable + " - " + syst[i / 2], variable + " - " + syst[i / 2], 700, 900));
        c->cd();
        ...
        hCent->GetYaxis()->SetRangeUser(minRatio - 0.02 * (maxRatio - minRatio),
                                        maxRatio + 0.02 * (maxRatio - minRatio));
                          ...
                          double integratedLumi)
{
    ...
    TCanvas *crossSectionPlot = makeCrossSectionPlot(lepSel,
                                                     integratedLumi,
    ...
    createTable(outputFileName + "_withLERS",
                ...
                true);
    createTable(
        outputFileName, lepSel, TString("ZNGoodJets_Zinc"), doNormalized, hInc, hCovInc, false);
}

int UnfoldData(const SectionedConfig &unfCfg,
               ...
               const char *outputFileName)
{
    ...
    if (!isSameBinning(*hr.GetXaxis(), *hRecDataMinusFakes->GetXaxis())) {
        std::cerr
            << __FILE__ << ":" << __LINE__
            << ". Fatal error: axes of measured and hronse matrix histograms are not conistents.\n";
        abort();
    }

    if (!isSameBinning(*hr.GetXaxis(), *hr.GetYaxis())) {
        std::cerr << __FILE__ << ":" << __LINE__
                  << ". Fatal error: x- and y- axes of the response matrices differs. If it's on "
                     "purpose, please edit the code to suppress this sanity check.\n";
        abort();
    }
    ...
    std::unique_ptr<TFile> f(new TFile(
        unfoldCheckDir + "/" + lepSel + "_" + variable + "_" + name + ".root", "RECREATE"));
    f->cd();
    ...
    if (xvalIter == 6 && nFixedIters < 1) {
        std::cerr
            << "Fatal error. Configuration error for " << lepSel << "_" << variable
    ...
    if (chosenIter <= 0) {
        ...
        hchi2->SetTitle(TString::Format("Reco #chi^{2}/ndf for %s", variable));
        ...
        hchi2dataMC->SetTitle(
            TString::Format("#chi^{2}/ndf test of MC gen and unfolded data for %s", variable));

        TH1D *hchi2Toy =
            new TH1D("hchi2Toy", "hchi2Toy", nTestIterMax + 1, -0.5, nTestIterMax + .5);
        hchi2Toy->SetTitle(TString::Format("Reco #chi^{2}/ndf for %s", variable));
        ...
        TH1D *hchi2ToyMcErr =
            new TH1D("hchi2ToyMcErr", "hchi2ToyMcErr", nTestIterMax + 1, -0.5, nTestIterMax + .5);
        hchi2ToyMcErr->SetTitle(TString::Format("Reco #chi^{2}/ndf for %s", variable));
        ...
        TH1D *hchi2ToyII =
            new TH1D("hchi2ToyII", "hchi2ToyII", nTestIterMax + 1, -0.5, nTestIterMax + .5);
        hchi2ToyII->SetTitle(TString::Format("Gen. #chi^{2}/ndf for %s", variable));
        ...
        if (hRecDataMinusFakesOdd && hRecDataMinusFakesEven) {
            ...
            hchi2Xval->Reset();
            hResMaxXval =
                new TH1D("hResMaxXval",
                         TString::Format("Max(#chi^{2}_{ibin}) for %s - Two samples;number of "
                                         "iterations;max(#chi^{2}_{ibin})",
                                         variable),
                         nTestIterMax + 1,
                         -.5,
                         nTestIterMax + 0.5);
        ...
        TH1 *hResMaxToyII = new TH1D(
            "hResMaxToyII",
            TString::Format("Gen. Max(#chi^{2}_{ibin}) for %s;Iter;max(#chi^{2}_{ibin})", variable),
            hUnfs->size(),
            -.5,
            hUnfs->size() - 0.5);
        hResMaxToyII->SetLineWidth(2);

        TH1 *hBiasMaxToyII = new TH1D(
            "hBiasMaxToyII",
            TString::Format("Gen. Max(|pull_{ibin}|) for %s;Iter;max(|pull_{ibin}|)", variable),
            hUnfs->size(),
            -.5,
            hUnfs->size() - 0.5);
        hBiasMaxToyII->SetLineWidth(2);

        double chi2dataMCThr =
            TH1Chi2Test(hRecDataMinusFakes, hRecDYJets, nFirstBinsToSkip, nLastBinsToSkip);
        ...
        for (unsigned i = 0; i < hUnfs->size(); ++i) {
            ...
            if (i == 0) {
                (*hUnfs)[i]->SetTitle(
                    TString::Format("Prior for %s %s %d iters", variable, name.Data(), i));
            } else {
                (*hUnfs)[i]->SetTitle(
                    TString::Format("Unfolded %s %s %d iters", variable, name.Data(), i));
            }
            (*hUnfs)[i]->Write();
            ...
            TH1 *hRes =
                (TH1 *)hRecDataMinusFakes->Clone(TString::Format("hRes%s_%d", name.Data(), i + 1));
            hRes->Reset();
            hRes->SetTitle(
                TString::Format("%s %s residuals - iteration %d", variable, name.Data(), i + 1));
            hRes->GetYaxis()->SetTitle("Residuals");
            const int nRes = hfoldUnfData->GetNbinsX();
            Double_t res[nRes];
            double mychi2 = MyChi2Test(hRecDataMinusFakesBis,
                                       hfoldUnfData,
                                       nFirstBinsToSkip,
                                       nLastBinsToSkip,
                                       res,
                                       hRecDataMinusFakesBis,
                                       false);
            if (verbosity)
            ...
            if (mychi2 < chi2Thr && finalNIter < 0) {
                ...
                finalNIter = nIter;
                std::cout << "Single distribution chi^2 leads to " << nIter
                          << " iterations with a final Chi2/ndf of: " << mychi2 << std::endl;
            }
            prevChi2 = mychi2;
            ...
            if (i == 0) {
                dataMcChi2 =
                    TH1Chi2Test(hRecDYJets, hRecDataMinusFakes, nFirstBinsToSkip, nLastBinsToSkip);
            } else {
                dataMcChi2 =
                    TH1Chi2Test(hGenDYJets, (*hUnfs)[i], nFirstBinsToSkip, nLastBinsToSkip);
            }
        ...
        if (hRecDataMinusFakesOdd && hRecDataMinusFakesEven) {
            ...
            for (unsigned i = 0; i < hUnfs2->size(); ++i) {
                ...
                if (i == 0) {
                    (*hUnfs2)[i]->SetTitle(
                        TString::Format("Prior for %s %s", variable, name.Data()));
                } else {
                    (*hUnfs2)[i]->SetTitle(TString::Format(
                        "Unfolded %s %s %d iters, cross-validation", variable, name.Data(), i));
                }
                (*hUnfs2)[i]->Write();
                ...
                hfoldUnfDataOdd->SetTitle(
                    TString::Format("dataOddFoldedBack%s_%d", name.Data(), i));
                hfoldUnfDataOdd->Write();

                TH1 *hResXval = (TH1 *)hRecDataMinusFakes->Clone(
                    TString::Format("hResXval%s_%d", name.Data(), i + 1));
                hResXval->Reset();
                hResXval->SetTitle(
                    TString::Format("%s %s residuals - iteration %d, cross validation",
                                    variable,
                                    name.Data(),
                                    i + 1));
                ...
                if (i > 0 && mychi2 < chi2XvalThr && nIterXval < 0) {
                    nIterXval = i;
                    // finalNIterXval = i;
                    std::cout << "Cross-validation chi^2 leads to " << nIter
                              << " iterations with a final Chi2/ndf of: " << mychi2 << std::endl;
                }
        ...
        for (int i = 0; i < nTestIterMax + 1; ++i) {
            ...
            if (bins) {
                ...
                hResToys[i] = new TProfile(
                    n,
                    t,
                    hRecDataMinusFakes->GetNbinsX(),
            ...
            if (bins) {
                hResToysII[i] =
                    new TProfile(n, t, hRecDataMinusFakes->GetNbinsX(), bins->GetArray(), "S");
            } else {
                hResToysII[i] = new TProfile(
                    n,
                    t,
                    hRecDataMinusFakes->GetNbinsX(),
                    hRecDataMinusFakes->GetXaxis()->GetBinLowEdge(1),
        ...
        std::vector<double> chi2Toy = chi2FromToy(alg,
                                                  smoothPrior,
                                                  resp,
                                                  hRecDataMinusFakes,
                                                  nFirstBinsToSkip,
        ...
        for (int iter = 0; iter <= nTestIterMax; ++iter) {
            double x = 0;
            TH1 *h = hResToys[iter];
            for (int ibin = 1 + nFirstBinsToSkip; ibin <= h->GetNbinsX() - nLastBinsToSkip;
                 ++ibin) {
            ...
            for (int ibin = 1 + nFirstBinsToSkip; ibin <= h->GetNbinsX() - nLastBinsToSkip;
                ...
                if (c > bias) {
                    ...
                    if (h->GetBinContent(ibin) > 0)
                        ebias = h->GetBinError(ibin) / sqrt(h->GetBinContent(ibin));
                    else
                        ebias = 0;
                }
        ...
        for (int iter = 0; iter < nTestIterMax + 1; ++iter) {
            ...
            if (iter > 0 && chi2Toy[iter] < chi2ToyThr && nIterToy < 0) {
                nIterToy = iter;
                std::cout << "Toy method lead to " << nIterToy
                          << " iterations with a final Chi2/ndf of: " << chi2Toy[iter] << std::endl;
            }
        ...
        if (nIterXval < 0) {
            std::cout << "Chi2/ndf from xval is always larger than 1. Using minimum as number of "
                         "iterations.\n";
            nIterXval = nIterXvalMin;
        }

        if (nIterToy < 0) {
            std::cout << "Chi2/ndf from toy is always larger than 1. Using minimum as number of "
                         "iterations.\n";
        ...
        if (xvalIter == 9) {
            ...
            if (nIterResMaxToyMin < nTestIterMax)
                chosenIter = nIterResMaxToyMin;
            else if (nIterResMax > 0 && nIterResMax < nTestIterMax)
        ...
        std::cout << "Bayes unfold, choice of number of iterations:\n"
                     "\tmin: "
                  << minIter << "\tmax: " << maxIter << "\tmode: " << mode_names[xvalIter]
        ...
        makeChi2Plot(hchi2,
                     ...
                     chosenAlgoThr,
                     unfoldCheckDir,
                     lepSel + "_" + variable + "_" + name + "_" + algo + "_chi2",
                     "cChi2",
                     chi2Ylog);

        if (hRecDataMinusFakesOdd && hRecDataMinusFakesEven) {
            hchi2Xval->Write();
            makeChi2Plot(hchi2Xval,
                         xvalIter == 1 ? -1 : nIterXval,
                         chosenIter,
                         chi2XvalThr,
                         chosenAlgoThr,
                         unfoldCheckDir,
                         lepSel + "_" + variable + "_" + name + "_" + algo + "_chi2Xval",
                         "cChi2Xval",
                         chi2Ylog);
        }
        ...
        std::cout << nIterToy << ", " << chosenIter << ", " << chi2ToyThr << ", " << chosenAlgoThr
                  << "\n";
        makeChi2Plot(hchi2Toy,
                     xvalIter == 2 ? -1 : nIterToy,
                     chosenIter,
                     chi2ToyThr,
                     chosenAlgoThr,
                     unfoldCheckDir,
                     lepSel + "_" + variable + "_" + name + "_" + algo + "_chi2Toy",
                     "cChi2Toy",
                     chi2Ylog);

        makeChi2Plot(hResMax,
                     xvalIter == 3 ? -1 : nIterResMax,
                     chosenIter,
                     resMaxThr,
                     chosenAlgoThr,
                     unfoldCheckDir,
                     lepSel + "_" + variable + "_" + name + "_" + algo + "_hResMax",
                     "cResMax",
                     chi2Ylog);

        if (hResMaxXval) {
            makeChi2Plot(hResMaxXval,
                         xvalIter == 4 ? -1 : nIterResMaxXval,
                         chosenIter,
                         resMaxXvalThr,
                         chosenAlgoThr,
                         unfoldCheckDir,
                         lepSel + "_" + variable + "_" + name + "_" + algo + "_hResXval",
                         "cResMaxXval",
                         chi2Ylog);
        }
        ...
        if (xvalIter == 6) {
            makeChi2Plot(hResMaxXval,
                         ...
                         -1,
                         unfoldCheckDir,
                         lepSel + "_" + variable + "_" + name + "_" + algo + "_hFixed",
                         "cFixed",
                         chi2Ylog);
        }
        makeChi2Plot(hResMaxToy,
                     xvalIter == 5 ? -1 : nIterResMaxToy,
                     chosenIter,
                     resMaxToyThr,
                     chosenAlgoThr,
                     unfoldCheckDir,
                     lepSel + "_" + variable + "_" + name + "_" + algo + "_hResMaxToy",
                     "cResMaxToy",
                     chi2Ylog);
        ...
        double thrline = -1;
        if (3 <= xvalIter && xvalIter <= 5)
            thrline = chosenAlgoThr; // the algo uses a thresold on this curve
        makeChi2Plot(hResMaxToy,
                     narr1,
                     chosenIter,
                     -1,
                     thrline,
                     unfoldCheckDir,
                     lepSel + "_" + variable + "_" + name + "_" + algo + "_hResMaxToyCmp",
                     "cResMaxToyCmp",
                     chi2Ylog,
                     altH,
                     labels);
        ...
        hchi2ToyII->Write();
        makeChi2Plot(hchi2ToyII,
                     nIterToyII,
                     chosenIter,
                     chi2ToyIIThr,
                     chosenAlgoThr,
                     unfoldCheckDir,
                     lepSel + "_" + variable + "_" + name + "_" + algo + "_chi2ToyII",
                     "cChi2ToyII",
                     chi2Ylog);

        makeChi2Plot(hResMaxToyII,
                     nIterResMaxToyIIMin,
                     chosenIter,
                     0.,
                     chosenAlgoThr,
                     unfoldCheckDir,
                     lepSel + "_" + variable + "_" + name + "_" + algo + "_resMaxToyII",
                     "cResMaxToyII",
                     chi2Ylog);
        ...
        makeChi2Plot(hchi2ToyIICpy,
                     nIterToyII,
                     chosenIter,
                     chi2ToyIIThr,
                     chosenAlgoThr,
                     unfoldCheckDir,
                     lepSel + "_" + variable + "_" + name + "_" + algo + "_chi2AndResMaxToyII",
                     "cChi2AndResMaxToyII",
                     chi2Ylog,
                     altH,
                     labels,
        ...
        makeChi2Plot(hchi2Toy,
                     ...
                     chosenAlgoThr,
                     unfoldCheckDir,
                     lepSel + "_" + variable + "_" + name + "_" + algo + "_chi2All",
                     "cChi2All",
                     chi2Ylog,
                     altH,
                     labels);
        ...
        makeChi2Plot(hchi2,
                     ...
                     chi2Thr,
                     unfoldCheckDir,
                     lepSel + "_" + variable + "_" + name + "_" + algo + "_chi2AndToyChi2",
                     "cChi2AndToyChi2",
                     chi2Ylog,
                     altH,
                     labels);
        ...
        labels.resize(altH.size() + 1);
        labels[0] = "Data vs MC";
        makeChi2Plot(hchi2dataMC,
                     -1,
                     chosenIter,
                     chi2dataMCThr,
                     chi2ToyThr,
                     unfoldCheckDir,
                     lepSel + "_" + variable + "_" + name + "_" + algo + "_chi2DataMC",
                     "cChi2DataMC",
                     chi2Ylog,
                     altH,
                     labels);

        makeChi2Plot(hBiasMaxToyII,
                     nIterBiasToyIIMin,
                     chosenIter,
                     -1,
                     chosenAlgoThr,
                     unfoldCheckDir,
                     lepSel + "_" + variable + "_" + name + "_" + algo + "_bias",
                     "bias",
                     chi2Ylog);
    ...
    if (tsvd_unfold) {
        std::cout << "----- RUN TSVD --- \n";
        std::unique_ptr<TSVDUnfold> unfoldTSVD(new TSVDUnfold(hRecDataMinusFakes,
                                                              (TH1D *)resp->Htruth(),
                                                              (TH1D *)resp->Hmeasured(),
                                                              (TH2D *)resp->Hresponse()));
        ...
        hmodD->SetTitle(hmodDOriginal->GetTitle() + TString(" for ") +
                        TString(hRecDataMinusFakes->GetTitle()));
        ...
        TArrow *arrowSvd =
            new TArrow(svdKterm, 20, svdKterm, 1.1 * hmodD->GetBinContent(svdKterm), 0.02, "|>");
    ...
    if (invert_unfold) {
        ...
        std::unique_ptr<RooUnfold> RObjectForDataInvert(
            RooUnfold::New(RooUnfold::kInvert, resp, hRecDataMinusFakes));
    ...
    bool unfSmoothPrior = cfg.getB("unfSmoothPrior", false);
    ...
    RObjectForData->IncludeSystematics(
        0); // new version of RooUnfold: will compute Cov based on Data Statistics only
    ...
    if (algo == "Bayes") {
        ...
        hUnfDataStatCov =
            M2H(RObjectForData->Ereco(RooUnfold::kCovariance)); // new version of RooUnfold
        ...
        RObjectForMC->IncludeSystematics(
            2); // new version of RooUnfold: will compute Cov based on MC Statistics only
        hUnfMCStatCov =
            M2H(RObjectForMC->Ereco(RooUnfold::kCovariance)); // new version of RooUnfold
        hUnfMCStatCov->SetName("UnfMCStatCov" + name);
        hUnfMCStatCov->SetTitle(hRecDataMinusFakes->GetTitle());

        TH1 *hUnfStdUnc;
        TGraphAsymmErrors *gr = statFromToy(RooUnfold::kBayes,
                                            smoothPrior,
                                            resp,
                                            hRecDataMinusFakes,
                                            chosenIter,
                                            ntoys,
                                            &hUnfStdUnc);
        TFile *f = new TFile(unfoldCheckDir + "/" + lepSel + "_" + variable + "_" + name + ".root",
                             "UPDATE");
        gr->Write(gr->GetName());
    ...
    for (int i = 1; i <= nBins; ++i) {
        ...
        for (int j = 1; j <= nBins; ++j) {
            if (hUnfDataStatCov) {
                hUnfDataStatCov->SetBinContent(
                    i, j, hUnfDataStatCov->GetBinContent(i, j) * 1. / (binWidth * binWidth));
                hUnfDataStatCov->SetBinError(
                    i, j, hUnfDataStatCov->GetBinError(i, j) * 1. / (binWidth * binWidth));
            }
            if (hUnfMCStatCov) {
                hUnfMCStatCov->SetBinContent(
                    i, j, hUnfMCStatCov->GetBinContent(i, j) * 1. / (binWidth * binWidth));
                hUnfMCStatCov->SetBinError(
                    i, j, hUnfMCStatCov->GetBinError(i, j) * 1. / (binWidth * binWidth));
            }
        }
    }
    return chosenIter;
...
TH2D *M2H(const TMatrixD m)
{
    ...
    TH2D *h =
        new TH2D(m.GetName(), m.GetName(), nBinsX - 2, 0, nBinsX - 2, nBinsY - 2, 0, nBinsY - 2);
...
TH2D *makeCovFromUpAndDown(const TH1D *hUnfDataCentral,
                           const TH1D *hUnfDataUp,
                           const TH1D *hUnfDataDown,
                           TString name,
                           bool noNull)
{
    ...
    for (int i = 1; i <= nBins; ++i) {
        double sigma_i =
            0.5 * scale * fabs(hUnfDataUp->GetBinContent(i) - hUnfDataDown->GetBinContent(i));
        ...
        if (noNull && i > 1 && i < nBins) {
            if ((hUnfDataUp->GetBinContent(i - 1) - hUnfDataDown->GetBinContent(i - 1)) *
                    (hUnfDataUp->GetBinContent(i + 1) - hUnfDataDown->GetBinContent(i + 1)) <
                0) {
                sigma_i = 0.5 * scale * (0.5 * fabs(hUnfDataUp->GetBinContent(i - 1) -
                                                    hUnfDataDown->GetBinContent(i - 1)) +
                                         0.5 * fabs(hUnfDataUp->GetBinContent(i + 1) -
                                                    hUnfDataDown->GetBinContent(i + 1)));
        ...
        for (int j = 1; j <= nBins; ++j) {
            double sigma_j =
                0.5 * scale * fabs(hUnfDataUp->GetBinContent(j) - hUnfDataDown->GetBinContent(j));
            // variation directions used together with sign_i to determine the sign of the
            // correlation coefficient:
            int sign_j =
                (hUnfDataUp->GetBinContent(j) - hUnfDataDown->GetBinContent(j) < 0) ? -1 : 1;
            if (noNull && j > 1 && j < nBins) {
                if ((hUnfDataUp->GetBinContent(j - 1) - hUnfDataDown->GetBinContent(j - 1)) *
                        (hUnfDataUp->GetBinContent(j + 1) - hUnfDataDown->GetBinContent(j + 1)) <
                    0) {
                    sigma_j = 0.5 * scale * (0.5 * fabs(hUnfDataUp->GetBinContent(j - 1) -
                                                        hUnfDataDown->GetBinContent(j - 1)) +
                                             0.5 * fabs(hUnfDataUp->GetBinContent(j + 1) -
                                                        hUnfDataDown->GetBinContent(j + 1)));
            ...
            if (cov) {
                for (int k = 0; k <= nBins; ++k) {
                    for (int l = 0; l <= nBins; ++l) {
                        error2 +=
                            hres->GetBinContent(i, k) * hres->GetBinContent(i, l) * (*cov)(l, k);
                    }
                }
            } else {
...
void test()
{
    ...
    std::cout << "reco: " << hreco->GetBinContent(1) << "   " << hreco->GetBinContent(2)
              << std::endl;
    std::cout << "gen : " << hgen->GetBinContent(1) << "   " << hgen->GetBinContent(2) << std::endl;
    std::cout << "unf : " << hUnfData->GetBinContent(1) << "   " << hUnfData->GetBinContent(2)
              << std::endl;
    std::cout << "fol : " << hfoldUnfData->GetBinContent(1) << "   "
              << hfoldUnfData->GetBinContent(2) << std::endl;
}

double MyChi2Test(const TH1 *h1,
                  const TH1 *h2,
                  int nFirstBinsToSkip,
                  int nLastBinsToSkip,
                  Double_t *res,
                  const TH1 *herr,
                  bool poisErr)
...
double
TH1Chi2Test(const TH1 *h1, const TH1 *h2, int nFirstBinsToSkip, int nLastBinsToSkip, Double_t *res)
{
...
void RemoveFakes(TH1 *hRecData, TH1 *hFakes, TH1 *hPurity)
{
    ...
    default:
        std::cerr << __FILE__ << ":" << __LINE__ << ". Value " << fakeMethod
                  << " for parameter fakeMethod "
                  << " is not supported. Exists.\n";
        exit(1);
    }
}
...
double pValueToNormChi2(double alpha, int n)
{
    ...
    double x_corr = 0;
    if (x > dx)
        x_corr = (TMath::Prob(x - dx, n) - alpha) / (TMath::Prob(x, n) - TMath::Prob(x - 0.01, n)) *
                 0.01;
    return (x + x_corr) / n;
}

TH1 *unfold(RooUnfold::Algorithm algo,
            const RooUnfoldResponse *resp,
            const TH1 *hRecDataMinusFakes,
            int niters,
            bool smoothPrior,
            ...
            int uncMode)
{
    ...
    rooUnfold->IncludeSystematics(
        uncMode); // new version of RooUnfold: will compute Cov based on Data Statistics only
    // The following Hreco call will trigger the data unfolding
    TH1 *r = rooUnfold->Hreco(RooUnfold::kCovariance, hUnfs);
    return r;
}

TH1 *unfoldWithErr(RooUnfold::Algorithm algo,
                   const RooUnfoldResponse *resp,
                   const TH1 *hRecDataMinusFakes,
                   int niters,
                   bool smoothPrior,
                   ...
                   int uncMode)
{
    ...
    if (hUnfs) {
        hUnfs->clear();
        // prior:
        TH1D *hPrior = (TH1D *)resp->Htruth()->Clone(
            TString::Format("%s_prior", hRecDataMinusFakes->GetName()));
    ...
    for (int i = 1; i <= niters; ++i) {
        ...
        rooUnfold->SetVerbose(verbosity);
        rooUnfold->UseFlatPrior(useFlatPrior);
        if (algo == RooUnfold::kBayes)
            ((RooUnfoldBayes *)rooUnfold.get())->SetSmoothing(smoothPrior);
        rooUnfold->IncludeSystematics(
            uncMode); // new version of RooUnfold: will compute Cov based on Data Statistics only
        // The following Hreco call will trigger the data unfolding
...
std::vector<double> chi2FromToy(RooUnfold::Algorithm algo,
                                bool smoothPrior,
                                const RooUnfoldResponse *resp,
                                const TH1 *hmeas,
                                int nFirstBinsToSkip,
                                ...
                                int niters,
                                int ntoys,
                                std::vector<double> *chi2mcErrs,
                                std::vector<double> *rms,
                                std::vector<TH1 *> *hChi2,
                                std::vector<TH1 *> *hChi2mcErr,
                                std::vector<double> *meanII,
                                std::vector<double> *rmsII,
                                std::vector<TH1 *> *hChi2II,
                                std::vector<TProfile *> *hRes,
                                std::vector<TProfile *> *hResMcErr,
    ...
    for (int itoy = 0; itoy < ntoys; ++itoy) {
        ...
        for (int iter = 0; iter <= niters; ++iter) {
            ...
            if (iter == 0)
                ...
                hrefold = foldUnfData(hUnfs->at(iter), &(covs[iter]), resp);
            ...
            double chi2 =
                MyChi2Test(hmeas_, hrefold, nFirstBinsToSkip, nLastBinsToSkip, res, herr, false);
            double chi2mcErr = MyChi2Test(
                hmeas_, hrefold, nFirstBinsToSkip, nLastBinsToSkip, resMcErr, herrMcErr, false);
            acc[iter] += chi2;
            acc2[iter] += chi2 * chi2;
            ...
            for (int iRes = 0; iRes < nRes; ++iRes) {
                ...
                if (hResMcErr)
                    hResMcErr->at(iter)->Fill(hResMcErr->at(iter)->GetBinCenter(1 + iRes),
                                              pow(resMcErr[iRes], 2));
            }
...
#else
                double chi2II = MyChi2Test(
                    newUnf, newUnf2, nFirstBinsToSkip, nLastBinsToSkip, res, hRefs->at(iter), true);
#endif
#endif
...
TGraphAsymmErrors *statFromToy(RooUnfold::Algorithm algo,
                               bool smoothPrior,
                               const RooUnfoldResponse *resp,
                               const TH1 *hmeas,
                               int niters,
...
void makeChi2Plot(TH1 *hChi2,
                  int thisAlgoNIters,
                  int chosenNIters,
                  double thisAlgoThr,
                  double chosenAlgoThr,
                  const char *outDir,
                  const char *fileBaseName,
                  const char *canvasName,
                  int chi2Ylog,
                  std::vector<TH1 *> hAltChi2,
                  ...
                  bool limitYmax)
{
    ...
    if (hChi2->GetMaximum() > 100 && chi2Ylog == 2)
        chi2Ylog = 1;
    else
        chi2Ylog = 0;
    ...
    auto moveup = [=](double y, double up) {
        if (!chi2Ylog)
            return y + (ymax - ymin) * up;
        else
            return y * std::pow(ymax / ymin, up);
    };
    ...
    if (chosenNIters == thisAlgoNIters)
        arrY1 = moveup(arrY0, 0.01);
    else
        arrY1 = moveup(hChi2->GetBinContent(hChi2->GetXaxis()->FindBin(chosenNIters)), 0.01);
    arrY0 = moveup(arrY1, 0.05);
    TArrow *arrowChi2_chosen =
        new TArrow(chosenNIters + 0.01, arrY0, chosenNIters + 0.01, arrY1, 0.02, "|>");
...
void testWithForgesResp(TH1 *hUnf,
                        int nIters,
                        const char *outDir,
                        ...
                        int nLastBinsToSkip)
{
    ...
    TH2 *h2resp = new TH2D("resp", "resp", nbins, xmin, xmax, nbins, xmin, xmax);
    ...
    hchi2->SetTitle(TString::Format("Reco #chi^{2}/ndf for %s", hUnf->GetName()));
    hchi2->GetYaxis()->SetTitle("#chi^{2}/ndf");
    hchi2->GetYaxis()->SetTitleOffset(1.40);
    ...
    for (int iter = 1; iter <= nIters; ++iter) {
        ...
        double chi2 =
            MyChi2Test(hmeas, hrefold, nFirstBinsToSkip, nLastBinsToSkip, 0, hmeas, false);
        hchi2->SetBinContent(hchi2->GetXaxis()->FindBin(iter), chi2);
    }
    hchi2->Write();

    makeChi2Plot(hchi2, 0, 0, 0., 0., outDir, outfileBaseName, "cChi2");
}
