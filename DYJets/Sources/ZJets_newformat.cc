        ...
        if (pBonzaiHeaderChain) {
            ...
            if (f && !f->IsZombie()) {
                if (f->GetDirectory("tupel")->FindKey("BonzaiHeader")) {
                    ...
                    std::cerr << "Warning: the tree BonzaiHeader was not found "
                                 "in file "
                              << fullFileName << ". We will assume we run on a boabab "
                                                 "file and not Baobab->Bonzai "
                              << "acceptance correction will be considered. "
                                 "This message "
                                 "can be ignored "
                              << "if for this sample Boabab ntuples are usd as "
                                 "input.\n";
                }
            }
        }
        ...
        if (!f) {
            ...
            while (!feof(f)) {
                ...
                *p = 0;
                ...
                treePath += "/EventTree";
                // std::cout << "Adding path " << treePath << " to the tree
                // chain.\n";
                if (pEventTreeChain) pEventTreeChain->Add(treePath);
                ...
                if (pBonzaiHeaderChain && (isBonzai == Unknown)) {
                    ...
                    if (f && !f->IsZombie()) {
                        if (f->GetDirectory("tupel")->FindKey("BonzaiHeader")) {
                            ...
                            isBonzai = False;
                            std::cerr << "Warning: the tree BonzaiHeader was not "
                                         "found in file "
                                      ...
                                      << "acceptance correction will be considered. "
                                         "This message "
                                         "can be ignored "
                                      << "if for this sample Boabab ntuples are usd "
                                         "as input.\n";
                        }
                    }
                }
...
void ZJets::getMcNorm()
{
    ...
    if (fBonzaiHeaderChain.GetListOfFiles()->IsEmpty()) {
        ...
        int nheaders = fBonzaiHeaderChain.GetEntries(); // can be several in case
        // files were merged with
        // haddd
...
void ZJets::Init(bool hasRecoInfo, bool hasGenInfo)
{
    ...
    if (hasRecoInfo) {
        ...
        // fChain->SetBranchAddress("JetAk04PartFlav", &JetAk04PartFlav,
        // &b_JetAk04PartFlav);
