	 ...
	 */
	 ...
	 if (passesgenLeptonCut){
	    ...
	    if (nGoodGenJets >= 1) {
		  ...
		  if(EWKBoson.Pt() > 50.) RatioValue2 =  JZB_ptHigh_2Hratio_fit->GetBinContent(binNumber2);
		  else RatioValue2 = 1.;
		  //}
	       }
...
bool ZJets::setTriggerMask(){
   std::vector<std::string> triggersRunB = cfg.getVS("triggersRunB");
   std::vector<std::string> triggersRunC = cfg.getVS("triggersRunC");
   std::vector<std::string> triggersRunD = cfg.getVS("triggersRunD");
   std::vector<std::string> triggersRunE = cfg.getVS("triggersRunE");
   ...
   */
   ...
   std::cout << "Available triggers for " << branchName << ":"<<std::endl;
   for(unsigned i = 0; i < TrigHlt->size(); ++i){
      if((*TrigHlt)[i].size()) std::cout <<i<< ": " << (*TrigHlt)[i]<<std::endl;
   }
   std::cout << "\n";
    
   bool rc = true;
   //Assuming all Runs have the same # of triggers. Should be improved.
   for(unsigned i = 0; i < triggersRunB.size(); ++i){
      for(unsigned bit = 0; bit < TrigHlt->size(); ++bit){
	 if(triggersRunB[i] == (*TrigHlt)[bit])
	    triggerMaskRunB |= (1 <<bit);
	 if(triggersRunC[i] == (*TrigHlt)[bit])
	    triggerMaskRunC |= (1 <<bit);
	 if(triggersRunD[i] == (*TrigHlt)[bit])
	    triggerMaskRunD |= (1 <<bit);
	 if(triggersRunE[i] == (*TrigHlt)[bit])
	    triggerMaskRunE |= (1 <<bit);
	 if(triggersRunF[i] == (*TrigHlt)[bit])
	    triggerMaskRunF |= (1 <<bit);
	 if(triggersRunG[i] == (*TrigHlt)[bit])
