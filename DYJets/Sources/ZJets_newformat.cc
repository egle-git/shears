   ...
   for (Long64_t jentry = entry_start; jentry <= entry_stop; jentry += 1) {
	 ...
	 */
	 ...
	 if(add_w_ > 0) 
	  ...
	  && fileName.Index("MIX") >= 0 
	  && GNup > 5) 
	 weight *= mixingWeightsDY[GNup - 6]; 
      
      if (fileName.Index("SMu_8TeV_WJets") >= 0 && fileName.Index("MIX") >= 0 && 
	  GNup > 5) weight *= mixingWeightsWJ_SMu[GNup - 6]; 
...
*/
	 ...
	 for (unsigned short i(0); i < nTotgenLeptons; i++) {
	    ...
	    leptonStruct genLep(GLepBarePt->at(i), GLepBareEta->at(i), 
				GLepBarePhi->at(i), GLepBareE->at(i), 
				charge, 0, 0, 0, 0);
	    leptonStruct genLepNoFSR(GLepBarePt->at(i), GLepBareEta->at(i), 
				     GLepBarePhi->at(i), GLepBareE->at(i), 
				     charge, 0, 0, 0, 0);
		...
		((lepSel == "SMu" || lepSel == "SE") && charge == 0)) {
		  ...
		  tmpGenPho.SetPtEtaPhiM(GLepClosePhotPt->at(j), GLepClosePhotEta->at(j), 
					 GLepClosePhotPhi->at(j), 0.);
		  int used(0);
		  for (unsigned short k(0); k < usedGenPho.size(); k++){
		     if (j == usedGenPho[k]) used = 1;
		  }
		  if (deltaR(tmpGenPho.Phi(), tmpGenPho.Eta(), genLepNoFSR.v.Phi(), 
			     genLepNoFSR.v.Eta()) < 0.1 && !used){
		     genLep.v += tmpGenPho;
		     usedGenPho.push_back(j);
		  }
	       }   

	       if ( (genLep.v.Pt() >= lepPtCutMin && fabs(genLep.v.Eta()) <= 
		     0.1*lepEtaCutMax && genLep.charge != 0)  
	 ...
	 if (countTauS3 > 0 && fileName.Index("Tau") < 0 && 
	     fileName.Index("Sherpa") < 0 && fileName.Index("MG5") < 0) {        
	    passesTauCut = 0;
	...
	//=======================================================================================================//
	 ...
	 if((lepSel == "DMu" || lepSel == "DE") && nLeptons >= 2 && ngenLeptons >= 2){  
	    if(smearlepton){
	       ...
	       leptons[0].v.SetPtEtaPhiE(newLeptonPt, leptons[0].v.Eta(), leptons[0].v.Phi(),
					 leptons[0].v.E()*newLeptonPt/oldLeptonPt);
		    
	       oldLeptonPt = leptons[1].v.Pt();
	       genLeptonPt = genLeptons[1].v.Pt();
	       newLeptonPt = SmearLepPt(oldLeptonPt,genLeptonPt,smearlepton,smearFactor);
	       leptons[1].v.SetPtEtaPhiE(newLeptonPt, leptons[1].v.Eta(), leptons[1].v.Phi(),
	 ...
	 for (unsigned short i(0); i < nTotJets; i++) {
	    ...
	    if (!EvtIsRealData && lepSel == "SMu") {
	       BTagModification(RandGen->Rndm(), JetAk04Pt->at(i), JetAk04Eta->at(i),
				JetAk04PartFlav->at(i), passesBJets);
	    }

	    jetStruct jet(JetAk04Pt->at(i), JetAk04Eta->at(i), JetAk04Phi->at(i), 
	    ...
	    //CAG
	    if (passesLeptonCut && jet.v.Pt() >= 30 && 
		jetPassesEtaCut && jetPassesIdCut && jetPassesdRCut)  // no MVA cut
	       fill(puMVA, JetAk04PuMva->at(i), weight);

	    if (jetPassesPtCut && jetPassesEtaCut && jetPassesIdCut && 
		jetPassesMVACut && jetPassesdRCut) {
	       jets.push_back(jet);
	 ...
	 for (unsigned short i(0); i < nTotGenJets; i++){
	    jetStruct genJet(GJetAk04Pt->at(i), GJetAk04Eta->at(i), 
			     GJetAk04Phi->at(i), GJetAk04E->at(i), i, 0);
	 ...
	 if (passesgenLeptonCut){
	    ...
	    if (nGoodGenJets >= 1){
		  ...
		  double binNumber1 = JetsHT_2_Zinc1jetHratio_fit->GetXaxis()->FindBin(jetsHT);
	       ...
	       fill(genVisPt_Zinc1jetQun, fabs((genHadronicR+genEWKBoson).Pt()), commonGenWeight*RatioValue2, EvtWeights);
	    ...
	    if (nGoodGenJets >= 2) {
		  ...
		  double binNumber1 = JetsHT_2_Zinc2jetHratio_fit->GetXaxis()->FindBin(jetsHT);
	       ...
	       fill(genVisPt_Zinc2jetQun, fabs((genHadronicR+genEWKBoson).Pt()), commonGenWeight*RatioValue2, EvtWeights);
	    ...
	    if (nGoodGenJets >= 3){
		  ...
		  if(RatioValue2 > 2. || RatioValue2 < 0.5) RatioValue2 = 1.; 
	       ...
	       fill(genVisPt_Zinc3jetQun, fabs((genHadronicR+genEWKBoson).Pt()), commonGenWeight*RatioValue2, EvtWeights);
...
*/             
	 ...
	 if (nGoodJets >= 1){
	    ...
	    if(nEvents % 2) fill(VisPt_Zinc1jetQun_Odd, fabs((hadronicR+EWKBoson).Pt()), weight*RatioValue2);
	    else fill(VisPt_Zinc1jetQun_Even, fabs((hadronicR+EWKBoson).Pt()), weight*RatioValue2);
	 ...
	 if (nGoodJets >= 2){
	    ...
	    if(nEvents % 2) fill(VisPt_Zinc2jetQun_Odd, fabs((hadronicR+EWKBoson).Pt()), weight*RatioValue2);
	    else fill(VisPt_Zinc2jetQun_Even, fabs((hadronicR+EWKBoson).Pt()), weight*RatioValue2);
