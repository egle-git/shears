...
void emu_method_builder::build()
{
    ...
    for (const std::string &name : _histogram_names) {
        ...
        std::unique_ptr<TH1> wjet = _ll_mc_entry->get_single_proc(name, "W");
        if (wjet != nullptr) {
            util::logging::debug << "ll mc W+Jets events: " << wjet->Integral() << std::endl;
            all_bkg_mc->Add(wjet.get(), -1);
            util::logging::debug << "ll mc events after removing W+Jets: " << all_bkg_mc->Integral() << std::endl;
        }
        ...
        fout->cd();
        if (_est_qcd_from_same_sign) {
            util::logging::debug << "Saving estimated QCD histogram" << std::endl;
            _emu_qcd_estimation->Write((name+"_qcd").c_str());
        }
        ...
        util::logging::debug << "Closing the output file" << std::endl;
        fout->Close();
        ...
        for (data::mc_group &group : _ll_mc_entry->groups()) {
            for (data::mc_group::sample_data &sd : group.samples_data()) {
                if (sd.sample.name() == "DYJets" || sd.sample.name() == "WToLNu")
                    continue;
                ...
                std::unique_ptr<TFile> fout_single = std::unique_ptr<TFile>(new TFile((_output_dir_name + "/dyjets-"+sd.sample.name()+".root").c_str(), "RECREATE"));
                fout_single->cd();
...
void emu_method_builder::load()
{
    ...
    std::string emu_same_sign_input_dir = _opt.config["emu same sign location"].as<std::string>();
    ...
    _est_qcd_from_same_sign = (emu_same_sign_input_dir != "" && emu_same_sign_input_dir != "-" &&
                               boost::to_upper_copy<std::string>(emu_same_sign_input_dir) != "NONE");
    util::logging::debug << "Same-sign files provided: " << (_est_qcd_from_same_sign ? "True" : "False") << std::endl;

    _ll_data_entry = load_data(ll_input_dir);
    _ll_mc_entry = load_mc(ll_input_dir, 0, 1);
    _emu_data_entry = load_data(emu_input_dir);
    _emu_mc_entry = load_mc(emu_input_dir, 0, 1);
    if (_est_qcd_from_same_sign) {
        _emu_same_sign_data_entry = load_data(emu_same_sign_input_dir);
        _emu_same_sign_mc_entry = load_mc(emu_same_sign_input_dir, 0, 1);
    }
    ...
    else if (_est_qcd_from_same_sign)
        if (_lumi != _emu_same_sign_data_entry->lumi() || _emu_data_entry->lumi() != _emu_same_sign_data_entry->lumi())
            util::logging::error << "Integrated luminosity does not match between different files!" << std::endl;
}
...
void emu_method_builder::fill_upper_panel(const std::string &name)
{
    ...
    if (_current_channel == "ll") {
        _ll_mc_entry->draw(name, _lumi);
        _ll_data_entry->draw(name, _lumi, true);

        if (auto axis = _ll_mc_entry->get_x_axis(name, _lumi)) {
            format_upper_x_axis(*axis);
        }
        if (auto axis = _ll_data_entry->get_x_axis(name, _lumi)) {
            format_upper_x_axis(*axis);
        }
    }
    else if (_current_channel == "emu") {
        _emu_mc_entry->draw(name, _lumi);
        _emu_data_entry->draw(name, _lumi, true);

        if (auto axis = _emu_mc_entry->get_x_axis(name, _lumi)) {
            format_upper_x_axis(*axis);
        }
        if (auto axis = _emu_data_entry->get_x_axis(name, _lumi)) {
            format_upper_x_axis(*axis);
        }
    }
    else if (_current_channel == "est") {
        _ll_mc_entry->draw(name, _lumi);
...
void emu_method_builder::fill_legend(TLegend &legend, const std::string &name)
{
    ...
    if (_current_channel == "ll") {
        _ll_data_entry->add_to_legend(legend, name, _lumi);
        _ll_mc_entry->add_to_legend(legend, name, _lumi);
    }
    else if (_current_channel == "emu") {
        _emu_data_entry->add_to_legend(legend, name, _lumi);
        _emu_mc_entry->add_to_legend(legend, name, _lumi);
    }
    else if (_current_channel == "est") {
...
bool emu_method_builder::fill_lower_panel(const std::string &name)
{
    ...
    if (_current_channel == "ll") {
        ...
        den = _ll_data_entry->get(name, _lumi);
    }
    else if (_current_channel == "emu") {
        num = _emu_mc_entry->get(name, _lumi);
        den = _emu_data_entry->get(name, _lumi);
    }
    else if (_current_channel == "est") {
        num = _ll_mc_entry->get(name, _lumi);
    ...
    format_lower_y_axis(*_ratio->GetYaxis(),
                        _reversed ? "e#mu method/Simulation" : "Simulation/e#mu method");
...
void emu_method_builder::reset_drawing_state()
{
    ...
    if (_est_qcd_from_same_sign) {
        _emu_same_sign_mc_entry->reset_drawing_state();
        _emu_same_sign_data_entry->reset_drawing_state();
    }
    _ratio.reset();
}
...
std::unique_ptr<TH1> emu_method_builder::emu_method(const TH1* ll_mc_input)
{
    ...
    if (_est_qcd_from_same_sign) {
        // Estimate QCD from same-sign
        util::logging::debug << "Estimating emu QCD from same-sign distribution" << std::endl;
        _emu_qcd_estimation = _emu_same_sign_data_entry->get(_current_histo_name, _lumi);
        _emu_qcd_estimation->SetName((_current_histo_name+"_qcd").c_str());
        std::unique_ptr<TH1> same_sign_mc = _emu_same_sign_mc_entry->get(_current_histo_name, _lumi);
        _emu_qcd_estimation->Add(same_sign_mc.get(), -1);
        util::logging::debug << "QCD events before negative bin removal: " << _emu_qcd_estimation->Integral() << std::endl;
        remove_negative_bins(_emu_qcd_estimation);
        util::logging::debug << "QCD events after negative bin removal: " << _emu_qcd_estimation->Integral() << std::endl;
        _emu_qcd_estimation->Scale(1/0.57147108645);
        // Remove QCD from opposite-sign emu data
        util::logging::debug << "Removing estimated QCD from opposite-sign emu data" << std::endl;
        num->Add(_emu_qcd_estimation.get(), -1);
        util::logging::debug << "emu data events after removing QCD: " << num->Integral() << std::endl;
    }
