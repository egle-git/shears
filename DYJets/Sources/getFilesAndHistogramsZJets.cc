...
TFile *getFile(TString histoDir,
               TString lepSel,
               TString energy,
               TString Name,
               int jetPtMin,
               ...
               TString syst)
{
    ...
    if (lepSel == "Muons" || lepSel == "DMu_")
        lepSel = "DMu";
    else if (lepSel == "Electrons" || lepSel == "DE_")
        lepSel = "DE";
    ...
    TString trigCorr = "0";
    if (Name.Index("Data") == 0 || energy == "8TeV" || energy == "13TeV")
        trigCorr =
            "1"; // trigger correction is applied to data and MC at 8TeV but only to data at 7TeV
    ...
    std::cout << "Opening " << fileName << "."
              << std::endl; //<< "   --->   Opened ? " << File->IsOpen() << std::endl;
    if (!File->IsOpen()) {
        std::cerr << "Please check that you produced the following file. I was not able to open it."
                  << std::endl;
...
void getFiles(TString histoDir,
              TFile *Files[],
              TString lepSel,
              TString energy,
              TString Name,
              ...
              int jetEtaMax)
{
    ...
    vector<TString> Syst;
    if (Name.Index("Data") >= 0 || Name.Index("data") >= 0 ||
        Name.Index("DATA") >= 0) { // for data we have:
...
void getAllFiles(TString histoDir,
                 TString lepSel,
                 TString energy,
                 int jetPtMin,
                 int jetEtaMax,
                 ...
                 int nBg)
{
    ...
    for (unsigned short iBg = 0; iBg < nBg; ++iBg) {
        ...
        assert(j < NSamples);
        std::cout << __FILE__ << ":" << __LINE__ << ".  Includes background from "
                  << Samples[j].name << "\n";
...
void getAllHistos(TString variable,
                  TH1D *hRecData[3],
                  TFile *fData[3],
                  TH1D *hRecDYJets[13],
                  TH1D *hGenDYJets[11],
                  ...
                  TFile *fBg[][7],
                  int nBg,
                  RooUnfoldResponse *respDYJets[],
                  TH1D *hFakDYJets[18],
                  TH1D *hPurityDYJets[18])
{
    ...
    if (match) tmpVariable.Insert(variable.Index("_Z"), "Match");
    ...
    for (unsigned short iBg = 0; iBg < nBg; ++iBg) {
        ...
        for (unsigned short iSyst = 0; iSyst < 11; ++iSyst) {
            ...
            if (hRecSumBg[iSyst] == 0)
                hRecSumBg[iSyst] = (TH1D *)hRecBg[iBg][iSyst]->Clone();
            else {
                if (hRecSumBg[iSyst]->GetXaxis()->GetNbins() !=
                    hRecBg[iBg][iSyst]->GetXaxis()->GetNbins()) {
                    std::cerr << __FILE__ << ":" << __LINE__ << ". "
                              << "Histogram " << hRecSumBg[iSyst]->GetName()
                              << "for systematic index " << iSyst << " and background index " << iBg
    ...
    for(size_t iFill=0;iFill<nFill;iFill++){
      ...
      double
    weightRec=hRecDYJets[0]->GetBinContent(hRecDYJets[0]->FindBin(rapRec))/hRecDYJets[0]->GetEntries();
      double
    weightGen=hGenDYJets[0]->GetBinContent(hGenDYJets[0]->FindBin(rapGen))/hGenDYJets[0]->GetEntries();
      weightRec *= 5.0;
      weightGen *= 5.0;
...
void closeFile(TFile *&File)
{
    if (File) {
        ...
        if (cfg.getI("verbosity") > 1)
            cout << "Closing: " << File->GetName() << "   --->   Closed ? " << (!(File->IsOpen()))
                 << endl;
        delete File;
        File = 0;
    }
...
void closeFiles(TFile *Files[])
{
    if (Files[0]) {
        ...
        int nFiles;
        if (fileName.Index("Data") >= 0 || fileName.Index("data") >= 0 ||
            fileName.Index("DATA") >= 0) {
            nFiles = 3;
        } else if (fileName.Index("DYJets") >= 0 && fileName.Index("UNFOLDING") >= 0 &&
                   fileName.Index("Tau") < 0) {
            nFiles = 9;
        } else
            nFiles = 7;
...
void closeFiles(TFile *Files[], int nFiles)
{
    ...
    for (int i(0); i < nFiles; i++) {
        ...
        closeFile(Files[i]);
        if (cfg.getI("verbosity") > 1)
            cout << "Closing file: " << Files[i]->GetName() << "   --->   Closed ? "
                 << (!(Files[i]->IsOpen())) << endl;
    }
}
...
void getHistos(TH1D *histograms[], TFile *Files[], TString variable)
{
    ...
    std::cerr << "getHistos(" << histograms << ", {" << Files[0]->GetName() << ",...}, " << variable
              << ")" << std::endl;

    TString fileName = gSystem->BaseName(Files[0]->GetName());
    bool isData =
        (fileName.Index("Data") >= 0 || fileName.Index("data") >= 0 || fileName.Index("DATA") >= 0);
    bool isSignal = (fileName.Index("DYJets") >= 0 && fileName.Index("UNFOLDING") >= 0 &&
                     fileName.Index("Tau") < 0);
    int nFiles = 0;
    ...
    if (dataDriven) {
        std::cout << "File " << Files[0]->GetName()
                  << " is expected to contain ttbar contribution estimated from data. "
...
void getHistos(TH2D *histograms[], TFile *Files[], TString variable)
{
    ...
    if (!histograms[0]) {
        std::cerr << "Central value histogran of observable " << variable << " was not found."
                  << " Aborting.\n";
...
void getResp(RooUnfoldResponse *response, TFile *File, TString variable)
{
    ...
    response = new RooUnfoldResponse(
        hRec, (TH1D *)File->Get("gen" + variable), (TH2D *)File->Get("hresponse" + variable));
...
RooUnfoldResponse *getResp(TFile *File, TString variable)
{
    ...
    if (!o) {
        std::cerr << __FILE__ << ":" << __LINE__ << ". Variable " << variable
                  << " was not found in file " << File->GetName() << "\n";
        return 0;
    }
...
void getResps(RooUnfoldResponse *responses[], TFile *Files[], TString variable)
{
    ...
    for (int i(0); i < nFiles; i++) {
        ...
        hRec->Reset();
        responses[i] = new RooUnfoldResponse(hRec,
...
TH1D *getFakes(TH1D *hRecDYJets, TH1D *hRecData, TH1D *hRecSumBg, TH2D *hResDYJets)
{
    ...
    for (int i = 0; i < nm; i++) {
        ...
        if (err2 < -1.e-6 * (pow(hRecDYJets->GetBinError(i), 2) + wmes)) {
            std::cerr << __FILE__ << ":" << __LINE__ << ". " << hRecDYJets->GetTitle() << ", bin "
                      << i << ": "
                      << "Uncertainty on n_tot (" << hRecDYJets->GetBinError(i)
                      << ") is smaller than the one on n_signal (" << sqrt(wmes) << ")!\n";
            err2 = 0;
        }
        // We neglect the uncertainty on the scale factor.
        hFakDYJets->SetBinError(i, sqrt(err2));
        // hFakDYJets->SetBinError   (i, sqrt (wmes + (sm ? pow(hRecDYJets->GetBinError(i),2) :
...
void getFakes(TH1D *hFakDYJets[18],
              TH1D *hRecData[3],
              TH1D *hRecSumBg[11],
              TH1D *hRecDYJets[13],
              TH2D *hResDYJets[13])
...
void getPurities(TH1D *hPurityDYJets[18],
                 TH1D *hRecData[3],
                 TH1D *hRecSumBg[11],
                 TH1D *hRecDYJets[13],
                 TH2D *hResDYJets[13])
...
void getResps(RooUnfoldResponse *responses[],
              TH1D *hRecDYJets[13],
              TH1D *hGenDYJets[11],
              TH2D *hResDYJets[13])
{
    if (hGenDYJets[0] == 0) {
        std::cerr << "\n"
                  << __FILE__ << ":" << __LINE__
                  << ". Error. Generator histogram pointer is null\n\n";
        return;
    }
...
void getStatistics(TString lepSel, int jetPtMin, int jetEtaMax, const TString &variable)
{
    ...
    nameStr << statDir << "/outputTable_" << lepSel << "_" << variable << "_JetPtMin_" << jetPtMin
            << "_JetEtaMax_" << jetEtaMax;
    nameStr << ".tex";
    ...
    for (int i = 1; i < usedFiles + 1; i++) {
        ...
        if (i < usedFiles)
            fprintf(outFile, " %s        & ", Samples[sel].legendReco.Data());
        else {
        ...
        for (int j = 1; j < NBins + 1; j++) {
            if (j < NBins)
                fprintf(outFile, "%d & ", int(DataEv[i][j]));
            else
                fprintf(outFile, "%d \\\\ \n ", int(DataEv[i][j]));
        }
        std::cout << std::endl;
    ...
    for (int j = 1; j < NBins + 1; j++) {
        if (j < NBins)
            fprintf(outFile, "%d & ", int(DataEv[0][j]));
        else
            fprintf(outFile, "%d \\\\ \n ", int(DataEv[0][j]));
    }
...
TString getUnfoldedFileName(TString unfoldDir,
                            const TString &lepSel,
                            const TString &variable,
                            const TString &algo,
                            int jetPtMin,
                            int jetEtaMax,
                            const TString &genList,
                            bool doNormalized)
{
...
std::vector<TH1 *> getGenHistos(const std::vector<std::string> samples,
                                const char *lepSel,
                                const char *variable,
                                bool xsec,
                                bool verbose)
{
    ...
    for (auto s : samples) {
        ...
        if (s.size() > 0) {
            ...
            for (auto l : {"DMu", "DE"}) {
                if (lepSel_ == l ||
                    lepSel_.Length() == 0) { // empty string used for ee/mumu channel average
                    ...
                    if (xsec && s != "DYJets_ZjNNLO") {
                        ...
                        if (!hLumi && verbose) {
                            std::cerr << "Error. Luminosity histogram required to normalized the "
                                         "histograms was not found in the file "
                                      << f->GetName() << ".\n";
                            continue;
                        }
                        ...
                        if (lumi <= 0.0) {
                            lumi = 35290.0;
                            std::cerr << "Warning. Problem with lumi value stored in "
                                      << f->GetName() << ". Integrated luminosity forced to "
                                      << lumi << " pb-1"
                                      << "\n";
                        }
                        // DJALOG The problem will be fixed in the next production for lumi in MC so
                        // this should
            ...
            if (h[i] && xsec &&
                s !=
                    "DYJets_ZjNNLO") { // DYJets_ZjNNLO histos are already divided by the bin widths
                // normalize to one-channel decay for cross-section histograms in case two channels
                // were sumed up
                h[i]->Scale(1. / ich);
