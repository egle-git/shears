...
void draw_axis_labels(TAxis *a)
{
    ...
    for (int ii = imin - 1; ii <= imax; ++ii) {
        double val = (ii == imin - 1) ? a->GetBinLowEdge(imin) : a->GetBinUpEdge(ii);
        float x_, y_;
        if (iV == 1) { // x-axis
            x_ = val;
            y_ = ymin - yoffset;
...
bool alignRanges(const TAxis *axref, TAxis *ax)
{
    ...
    while (first < ax->GetNbins() + 1 &&
           fabs(ax->GetBinLowEdge(first) - xmin) / axref->GetBinWidth(axref->GetFirst()) > eps)
        ++first;
    ...
    for (int i = 0; i <= axref->GetLast() - axref->GetFirst() + 1; ++i) {
        if (fabs(ax->GetBinLowEdge(ax->GetFirst() + i) -
                ...
                axref->GetBinWidth(axref->GetFirst() + i) >
            eps) {
            return false;
        }
    }
