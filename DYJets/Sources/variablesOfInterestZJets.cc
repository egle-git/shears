...
bool decomposeVarName(const std::string& name, std::string* observable, 
		      std::string* jetMultStr,
		      int* nJets, bool* isInc){
    ...
    if(rc != 0){
      std::cerr << "Bug found in " __FILE__ << ":" << __LINE__ << ": invalid regular expression.\n";
      char buffer[256];
    ...
    if(rc != 0){
      std::cerr << "Failed to interpret the variable name '"
		<< name << "'. It does not follow the common pattern '"
		<< p << "'.\n";
    } else{
      ...
      if(isInc) *isInc = (name.compare(match[3].rm_so, match[3].rm_eo - match[3].rm_so, "Zinc") == 0);
      if(nJets){
	if(match[4].rm_so < 0) *nJets = 0; //Zinc or Zexc without n jet specification => 0 jet
	else *nJets = strtol(name.substr(match[5].rm_so, match[5].rm_eo - match[5].rm_so).c_str(), 0, 10);
      }
    }    
    regfree(&reg);
    return (rc == 0);
