...
Unfold::Unfold(TH1F*hReco,TH1F*hTrue,TH2F*hMatrix,RegType regType=NO_REG)
{
    ...
    if(_trueVert){
        ...
        std::cout << "reco distribution is on the y-axis" << endl;
        std::cout << "true distribution is on the x-axis" << endl;
    }
...
void Unfold::unfoldTUnfold()
{
    ...
    unfold.GetEmatrix(_hEmatStat);
    _hOutputCovariance = _hEmatStat;
}//end unfoldTUnfold
...
TH2*Unfold::ReturnOutputCovariance()
{
